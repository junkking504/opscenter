...
import { normalizeTruckLoadLabel, readTruckLoadStatuses, readTruckLoadStatus, resetTruckLoad, setTruckStartingLoad, recordTruckLoadSnapshot } from '@/lib/truck-load-status';
import { completedTruckJobIds, readOperationalTruckLoads, truckChargeSummary } from './truck-load-closeouts';
...
export function readDesktopFleet(date:string, report:string, role:InteractiveOpsRole):DesktopFleetSnapshot {
  ...
  return {date,report,drivingScores:report==='scores'?readDesktopDrivingScores(date):undefined,sourceUpdatedAt:map?.lastUpdatedAt||null,sourceAvailable:Boolean(map),canWrite:opsRoleCan(role,'operations.write'),trucks:names.map(name=>{
    ...
    const inspection=inspectionFleetEvidence(inspections,name);
    const complete=Boolean(inspection)||Boolean(checklist?.completedAt&&checklist.inspector&&definitions.every(def=>checklist.answers.some(answer=>answer.itemId===def.itemId)));
    return {id:name,label:name,vehicle:source?.yearMakeModel||source?.vehicleName||'Unavailable',readiness:!issuesAvailable?'Unavailable':inspection?.stop||repairs.some(row=>row.severity==='out_of_service')?'Out of service':!complete||inspection?.problem||repairs.length?'Attention':'Ready',operatingStatus:source?.operationalStatus||'Unavailable',driver:source?.driver||'Unassigned',navigator:source?.navigator||'Unassigned',assignment:source?.currentOrHistoricalAppointment||'Unavailable',location:source?.hasCoordinates?`${source.latitude?.toFixed(4)}, ${source.longitude?.toFixed(4)}`:'Unavailable',gpsAt:source?.lastGpsUpdate||null,speed:source?.speed??null,ignition:source?.ignition||null,gpsFreshness:source?.freshnessLabel||'Unavailable',odometer:source?.odometer||'Unavailable',serviceStatus:source?.serviceStatus||'Unavailable',nextService:[source?.mileageUntilNextService,source?.daysUntilNextService].filter(Boolean).join(' · ')||'No source target',checklist:inspection?.label||(complete?'Complete':'Missing'),inspectionSource:inspection?.source,inspectionHref:inspection?.href,inspectionFindings:inspection?.findings||checklist?.answers.filter(answer=>answer.status==='attention'||answer.notes.trim()).map(answer=>({label:definitions.find(def=>def.itemId===answer.itemId)?.label||answer.itemId,notes:answer.notes.trim()||'Needs attention; no additional detail recorded.'}))||[],loadPercent:load?.events.length?load.capacityPercent:null,loadLabel:load?.displayLoadLabel||'Load not recorded',loadNeedsVerification:Boolean(load?.needsVerification),loadNote:load ? [truckChargeSummary(load),load.verificationNote || (load.isOverCapacity?'Over capacity · verify an unload or the recorded loads.':load.currentContents)].filter(Boolean).join(' ') : 'No load observation recorded',loadVersion:desktopVersion(storedLoads.find(row=>normalizeTruckLoadLabel(row.truck)===name)||null),checklistVersion:desktopVersion(checklist),checklists:{daily:checklistFor('daily'),weekly:checklistFor('weekly'),monthly:checklistFor('monthly')},checklistDefinitions:definitions,answers:checklist?.answers||[],jobs:source?.jobsCompleted??null,revenue:source?.revenue??null,miles:source?.milesDriven??null,idleMinutes:null,driverScore:source?.driverScore??null};
  }),issues:issues.map(row=>({...row,version:desktopVersion(row)})),maintenance:maintenance.map(row=>({...row,version:desktopVersion(row)})),reportRows:summary?.truckRows||[],reportCoverageDays:summary?.coverageDays||0,warnings:[...truckLoadTrackingAlerts(date,loads).map(alert=>`${alert.title}. ${alert.facts[1].value}`),...(!issuesAvailable?['Repair source is unavailable. Inspection completion alone does not establish readiness.']:[]),...(map?.mappingWarnings||[]),...(!map?['Fleet telemetry source is unavailable. Stored maintenance records do not establish current GPS status.']:[]),...(report==='reports'?['Report totals include available daily observations only; coverage days are shown.']:[])]};
}
