 ...
 try {for(const width of [320,390,430]){
  ...
  await page.route('**/api/**',async route=>{
   ...
   if(u.pathname==='/api/crew-jobs/closeout'){
    if(r.method()==='POST'){
     posts++;const b=r.postDataJSON();assert.equal(b.assignmentId,'sample-assignment');assert.equal(b.values.truck,'Truck 6');assert.equal(b.values.targetStatus,'8');assert.deepEqual(b.values.addPayment,{methodId:'3',amount:'400',reference:'1234'});
    ...
    return send({closeout:closeout(),sourceVersion,jobVersion:'sample-version',canWrite:true});
   }
  ...
  await expect(page.getByRole('radio',{name:'Cancelled',exact:true})).toHaveCount(0);
  ...
  sourceVersion='b'.repeat(64);await page.reload();await open();await page.getByRole('button',{name:'Continue to charges',exact:true}).click();await page.getByRole('button',{name:'Continue to payment',exact:true}).click();await expect(page.getByLabel('Record a collected payment',{exact:true})).not.toBeChecked();
