...
export default function SwitchTruck({day,trucks,pending,onDone,onCancel,onBusy}:{day:CrewPhoneDay;trucks:string[];pending:SwitchSummary|null;onDone:()=>void;onCancel:()=>void;onBusy:(value:boolean)=>void}){
 ...
 return <section className={styles.card}><h1>Switch truck</h1>
  ...
  {saved?<><h2>{saved.from} → {saved.to}</h2><p role="status">{saved.message}</p><p>{saved.moved} of {saved.total} unfinished jobs moved.</p>{!busy && <button className={styles.primary} onClick={()=>void run('continue')}>Check saved switch</button>}<p>Your crew and saved job progress stay with this phone.</p></>:<>
   <p>Current truck: <strong>{day.truck}</strong></p><label className={styles.form}>Replacement truck<select required disabled={busy} value={truck} onChange={e=>{setTruck(e.target.value);setPreview(null);request.current=null;}}><option value="">Choose a truck</option>{trucks.filter(t=>t!==day.truck).map(t=><option key={t}>{t}</option>)}</select></label>
   {preview?<><h2>{preview.from} → {preview.to}</h2><p>{preview.count} unfinished {preview.count===1?'job moves':'jobs move'} with your crew. Completed jobs stay with {preview.from}. Saved photos and closeout progress stay attached to their jobs.</p><p>{preview.inspection.status==='ready'?'This truck has already been inspected today. You can continue to jobs once the switch is verified.':preview.inspection.status==='blocked'?'Do not operate: this truck’s latest inspection blocks jobs. Contact your manager.':'Inspect this truck after switching, then continue to jobs.'}</p><button className={styles.primary} disabled={busy} onClick={()=>void run('confirm')}>Move our unfinished jobs to {preview.to}</button></>:<button className={styles.primary} disabled={busy || !truck} onClick={()=>void run('preview')}>Review switch</button>}
   <button className={styles.secondary} disabled={busy} onClick={onCancel}>Keep {day.truck}</button>
  </>}
