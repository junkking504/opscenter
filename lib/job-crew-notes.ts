...
function normalizeEntry(value: unknown): JobCrewNote | null {
  ...
  return {
    ...
    updatedBy: String(row.updatedBy || "").trim().slice(0, 200),
  };
}
  ...
  try {
    ...
    return {
      ...
      entries: (Array.isArray(payload?.entries) ? payload.entries : [])
        .map(normalizeEntry)
        .filter((entry: JobCrewNote | null): entry is JobCrewNote => Boolean(entry)),
    };
  } catch {
    return emptyStore();
  }
...
export function readJobCrewNotes(date?: string): Map<string, JobCrewNote> {
  return new Map(
    readStore().entries
      ...
      .map((entry) => [jobCrewNoteLookupKey(entry.date, entry.jobKey), entry]),
  );
}

export function saveJobCrewNote(input: {
  date: string;
  jobKey: string;
  appointmentId: string;
  body: string;
...
function crewAssignedTo(appointment: Record<string, unknown>, employee: string): boolean {
  ...
  return [
    ...
    appointment.driver,
    appointment.driver_normalized_name,
    appointment.navigator_name,
    appointment.navigator,
    appointment.navigator_normalized_name,
    appointment.crew,
    appointment.additional_crew,
  ].flatMap(people).includes(target);
}
...
export function crewJobNotesFromAppointments(
  employee: string,
  date: string,
  appointments: unknown,
  notes: Map<string, JobCrewNote>,
): CrewJobNote[] {
  ...
  for (const value of appointments) {
    ...
    visible.push({
      ...
      customerName: String(appointment.customer_name || appointment.customer || "Customer").trim() || "Customer",
      address: String(appointment.service_address || appointment.address || "Address unavailable").trim() || "Address unavailable",
      appointmentTime: String(appointment.appointment_time || appointment.time || "Time unavailable").trim() || "Time unavailable",
      truck: String(appointment.assigned_truck || appointment.truck || "Truck assignment unavailable").trim() || "Truck assignment unavailable",
    });
  }
