...
try {
  ...
  const expected = ['Arrival','New Appointment','Clock In','Duration','Job Completed'];
  ...
  assert.deepEqual(await labels(),['Duration','Job Completed']);
  await page.getByRole('button',{name:'Clear filters',exact:true}).click();
  ...
  assert.deepEqual(await labels(),['Arrival','New Appointment','Truck Unloaded','Clock In','Duration','Job Completed','Inspection'],'Source updates missing from appointment indexes are retained; timezone offsets sort by instant and unknown times sort last');
  await page.getByRole('button',{name:'Reset preview'}).click();
  await page.getByRole('button',{name:'Simulate warehouse entry'}).click();
  assert.deepEqual(await labels(),['Arrival','New Appointment','Geofence Entry','Clock In','Duration','Job Completed'],'Native geofence entries interleave by occurrence time');
