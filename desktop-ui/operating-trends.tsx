...
import { operatingChartRows, scopeOperatingTrends, scopeMetrics, type AnalyticsScope, type OperatingMetric, type OperatingTrendsData } from '../lib/operating-trends';
...
function OperatingChart({ data, metric, days }: { data: OperatingTrendsData; metric: typeof metrics[number]; days: number }) {
  const rows = operatingChartRows(data, metric.key, days);
  ...
  const ticks = [...new Set([rows[0]?.date, rows[Math.floor((rows.length - 1) / 2)]?.date, rows.at(-1)?.date].filter((value): value is string => Boolean(value)))];
  ...
  return <article className="operating-chart" aria-labelledby={`operating-${metric.key}`}>
    ...
    <div className="operating-chart-legend"><span><i />{metric.source}</span>{metric.key === 'fuelCost' && <span><i className="secondary" />JunkWare recorded fuel</span>}{forecast.length > 0 && <><span><i className="forecast" />Baseline forecast</span><span><i className="spread" />Historical spread</span></>}</div>
    {!hasHistory && !forecast.length ? <p className="operating-empty">No recorded data in this period.</p> : <div className="operating-chart-plot">
      <ResponsiveContainer width="100%" height="100%"><ComposedChart data={rows} margin={{ top: 12, right: 20, bottom: 0, left: 0 }} accessibilityLayer>
        ...
        <XAxis dataKey="date" ticks={ticks} tickFormatter={day} tickLine={false} axisLine={false} minTickGap={24} />
        ...
        <Tooltip labelFormatter={value => String(value)} formatter={(value, name) => [Array.isArray(value) ? value.map(item => amount(Number(item), metric.unit)).join(' – ') : amount(typeof value === 'number' ? value : null, metric.unit), name]} contentStyle={{ background: 'var(--card)', color: 'var(--foreground)', border: '1px solid var(--border)', borderRadius: 8 }} />
        {forecast.length > 0 && <ReferenceLine x={forecast[0].date} stroke="var(--muted-foreground)" strokeDasharray="3 4" />}
        ...
        <Line type="linear" dataKey="actual" name={metric.source} stroke="var(--operating-actual)" strokeWidth={2} dot={{ r: 2, fill: 'var(--operating-actual)', strokeWidth: 0 }} activeDot={{ r: 5 }} connectNulls={false} isAnimationActive={false} />
    ...
    <p className="operating-chart-latest">{latest ? `Latest ${metric.source}: ${amount(latest.actual, metric.unit)} · ${day(latest.date)}` : 'No observations in this history window.'}</p>
    ...
    <p>{metric.note}</p>
    <details><summary>View daily values & source</summary><p>{metric.source}. Dates use the source reporting day. Blank values mean unavailable, not zero.</p><div className="operating-data-table"><table><thead><tr><th>Date</th><th>{metric.source}</th>{metric.key === 'fuelCost' && <th>JunkWare recorded fuel</th>}{forecast.length > 0 && <><th>Baseline</th><th>Historical spread</th></>}</tr></thead><tbody>{rows.map(row => <tr key={row.date}><td>{row.date}</td><td>{amount(row.actual, metric.unit)}</td>{metric.key === 'fuelCost' && <td>{amount(row.secondary, metric.unit)}</td>}{forecast.length > 0 && <><td>{amount(row.forecast, metric.unit)}</td><td>{row.spread ? row.spread.map(value => amount(value, metric.unit)).join(' – ') : '—'}</td></>}</tr>)}</tbody></table></div></details>
  </article>;
}
...
export function OperatingTrends({ data, scope = 'expenses', focusMetric }: { data?: OperatingTrendsData | null; scope?: AnalyticsScope; focusMetric?: OperatingMetric }) {
  ...
  return <section className="operating-trends" aria-label={titles[scope]}>
    ...
    <div className={`operating-chart-grid${focusMetric ? ' operating-chart-focused' : ''}`}>{scopeMetrics[scope].filter(key => !focusMetric || key === focusMetric).map(key => metrics.find(metric => metric.key === key)!).map(metric => <OperatingChart key={metric.key} data={chartData} metric={metric} days={days} />)}</div>
