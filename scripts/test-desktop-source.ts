...
const metrics = { date: '2026-09-15', generated_at: '2026-09-15T18:00:00Z', total_revenue: 1200, total_payroll: 180, dump_expense: 125, fuel_expense: 75, total_expenses: 500, net_profit: 700, completed_jobs: 3, revenue_by_truck: { 'Truck 3': 800, 'Truck 9': 400 }, employee_leaderboard: [{ hours_worked: 4 }, { hours_for_rph: 6, hours_worked: 8 }] };
const cards = desktopCommandKpis(metrics, null, 2, undefined, '2026-09-15', Date.parse('2026-09-15T18:01:00Z'));
assert.deepEqual(cards.map(card => card.label), ['Today’s jobs', 'Revenue', 'Labor', 'Dump + Fuel', 'Net']);
assert.equal(cards[1].value, '$1,200.00');
assert.equal(cards[2].value, '$180.00');
assert.equal(cards[2].secondaryValue, '15.0% of revenue');
assert.equal(cards[3].value, '$200.00');
assert.equal(cards[3].secondaryValue, 'Dumps $125.00 · Fuel $75.00');
assert.equal(cards[4].value, '$700.00');
const assumedDumpCards = desktopCommandKpis(metrics, null, 2, undefined, '2026-09-15', Date.parse('2026-09-15T18:01:00Z'), 44);
assert.equal(assumedDumpCards[3].value, '$244.00');
assert.equal(assumedDumpCards[3].secondaryValue, 'Dumps $169.00 · Fuel $75.00');
assert.match(assumedDumpCards[3].detail, /Includes \$44\.00 assumed dump cost/);
assert.equal(assumedDumpCards[4].value, '$656.00');
assert.equal(desktopCommandKpis({ ...metrics, total_revenue: 0 }, null, 2, undefined, '2026-09-15', Date.parse('2026-09-15T18:01:00Z'))[2].value, '$180.00');
