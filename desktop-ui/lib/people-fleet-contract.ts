...
export type DesktopCrewMember = CrewAmounts & { id: string; name: string; initials: string; role: string; truck: string; working: boolean; clockIn: string; clockOut: string; hourlyRate: number | null; status: string; issue: string; payNote?: string; junkwareSync?: {status:'pending'|'verified'|'failed'|'uncertain';message:string;verifiedAt?:string}|null; version: string; actionVersions?: { correction: string; bonus: string }; correction: { clockIn: string; clockOut: string; hourlyRate: number; note: string; updatedBy: string; updatedAt: string } | null; days: Array<CrewAmounts & { date: string; clockIn: string; clockOut: string }> };
