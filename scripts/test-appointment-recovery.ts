...
async function main() {
  ...
  try {
    ...
    for(const test of ['rejected','preflight','missing-lane','unknown','other-day','newer-override']) {
      const id=String(2000+['rejected','preflight','missing-lane','unknown','other-day','newer-override'].indexOf(test));
      const requestId=randomUUID();
      const pending=saveJobRouteAssignment({date:op.date,jobKey:`appt:${id}`,appointmentId:id,truck:'Truck 1',appointmentStartMinutes:660,appointmentEndMinutes:720,junkwareSyncStatus:'pending',junkwareSyncError:test==='missing-lane'?'page.evaluate: Error: The requested JunkWare dispatch lane is unavailable.':test==='unknown'?'Save response lost':test==='preflight'?'page.evaluate: Error: This appointment is not draggable in the source daily schedule.':'11:00 AM is not available for this JunkWare appointment.'})!;
      ...
      assert.equal(resolved?.status,['rejected','preflight','missing-lane'].includes(test)?'failed':'uncertain',test);
      if(['rejected','preflight','missing-lane'].includes(test)) {
