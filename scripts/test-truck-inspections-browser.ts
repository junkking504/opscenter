...
async function main() {
  ...
  try {
    ...
    console.log("Browser acceptance passed: Figma mobile layout, double-tap guard, required fuel, operating decision reset on edit, stop receipt, status filters, manager setup, phone pairing, five checks, problem note/photo, offline draft reload, lost acknowledgement recovery, duplicate safety, report read-back, revoked device and management isolation.");
  } finally {
    await browser.close();
    server.kill("SIGTERM");
