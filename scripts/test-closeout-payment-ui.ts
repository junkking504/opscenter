...
async function main(){
 ...
 const recoveredPage=await browser.newPage({viewport:{width:390,height:844}});
 let recovered=false, recoveryReads=0, recoveryWrites=0;
 const closeoutReceipt={requestId:'fixture-recovery',action:'closeout',status:'uncertain',message:'Charge label requires verification.'};
 const recoveredSource={...fixture,status:field('8','Completed'),loadPrice:'100',total:'$103.00',balance:'0.00',otherCharges:[{label:'CC Surcharge (Card Present)',quantity:'1',price:'$3.00',total:'$3.00'}],payments:[{description:'CREDIT CARD, ***1234',amount:'$103.00'}]};
 await recoveredPage.route('**/api/desktop/schedule/closeout?*',route=>{recoveryReads++;return route.fulfill({json:{closeout:recovered?recoveredSource:fixture,sourceVersion:(recovered?'b':'a').repeat(64),canWrite:true,pendingReceipt:recovered?null:closeoutReceipt}});});
 await recoveredPage.route('**/api/desktop/schedule/operations?*',route=>{recovered=true;return route.fulfill({json:{receipt:{...closeoutReceipt,status:'reconciled',message:'Current source reviewed without replay.'}}});});
 await recoveredPage.route('**/api/desktop/schedule/operations',route=>{recoveryWrites++;return route.fulfill({status:400,json:{}});});
 await recoveredPage.goto(url);await recoveredPage.getByText('Appointment Closeout',{exact:true}).click();
 await recoveredPage.locator('.closeout-footer-slot').getByRole('button',{name:'Check Saved Result',exact:true}).click();
 await expect(recoveredPage.getByRole('textbox',{name:'Load price',exact:true})).toHaveValue('100');
 await expect(recoveredPage.getByRole('combobox',{name:'Final appointment category'})).toBeEnabled();
 await expect(recoveredPage.getByRole('checkbox',{name:'Add a payment'})).not.toBeChecked();
 await expect(recoveredPage.getByText('CREDIT CARD, ***1234',{exact:true})).toBeVisible();
 await expect(recoveredPage.getByText('Verification Required',{exact:true})).toHaveCount(0);
 assert.equal(recoveryReads,2,'Reconciled closeout reloads the current source and version');
 assert.equal(recoveryWrites,0,'Recovery never resubmits a closeout or payment');
