...
export async function GET(request:Request){try{
    ...
    if(testPhone.test)return crewPhoneResponse(waypointSandbox(testPhone,'switch-truck',new URL(request.url).searchParams));
...
export async function POST(request:Request){try{
    const testPhone=requireCrewPhone(request);
    if(testPhone.test)return crewPhoneResponse(waypointSandbox(testPhone,'switch-truck',new URL(request.url).searchParams,await crewPhoneBody(request,16384)));
