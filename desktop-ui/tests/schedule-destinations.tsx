...
function appointments(date:string):ScheduleAppointment[] {
  return Array.from({length:areaCases?.length ?? (scenario==='empty'?0:scenario==='crowded'?14:scenario==='dense'?24:scenario==='same-time'?4:['on-site','route-stack'].includes(scenario)?3:scenario==='truck-scroll'?2:1)},(_,index)=>{
    ...
    .map(job=>scenario==='gps-mismatch'?{...job,status:'Completed',truck:assignments.get(job.recordId)||'Truck 6',onsiteTime:{minutes:null,arrival:null,departure:null,label:'Unavailable · no confirmed visit'},recordedOnsiteTime:{minutes:32.5,arrival:'2026-09-08T14:06:00Z',departure:'2026-09-08T14:38:30Z',label:'32.5 min',truck:'Truck 3'},truckVisits:[{truck:'Truck 3',arrival:'2026-09-08T14:06:00Z',departure:'2026-09-08T14:38:30Z',observedThrough:'2026-09-08T14:38:30Z'}]}:job)
    .map(job=>scenario==='co-located'?{...job,truck:'Truck 6',customerName:'Shared appointment',location:{latitude:30.4,longitude:-91.16},truckVisits:[{truck:'Truck 3',arrival:'2026-09-08T14:24:00Z',departure:null,observedThrough:'2026-09-08T14:28:00Z'},{truck:'Truck 6',arrival:'2026-09-08T14:34:00Z',departure:null,observedThrough:'2026-09-08T14:36:00Z'}]}:job)
    .map(job=>scenario==='return-visit'?{...job,status:'Completed',truck:'Truck 8',appointmentTime:'9:00 AM–11:00 AM',appointmentStartMinutes:540,appointmentEndMinutes:660,recordedOnsiteTime:{minutes:60,arrival:'2026-09-08T14:00:00Z',departure:'2026-09-08T16:00:00Z',label:'60 min',truck:'Truck 8',intervals:[{arrival:'2026-09-08T14:00:00Z',departure:'2026-09-08T14:30:00Z'},{arrival:'2026-09-08T15:30:00Z',departure:'2026-09-08T16:00:00Z'}]},truckVisits:[{truck:'Truck 8',arrival:'2026-09-08T14:00:00Z',departure:'2026-09-08T14:30:00Z',observedThrough:'2026-09-08T14:30:00Z'},{truck:'Truck 8',arrival:'2026-09-08T15:30:00Z',departure:'2026-09-08T16:00:00Z',observedThrough:'2026-09-08T16:00:00Z'}],truckVisitGaps:[{truck:'Truck 8',departedAt:'2026-09-08T14:30:00Z',returnedAt:'2026-09-08T15:30:00Z',kind:'dump',facilityName:'Gentilly',facilityEnteredAt:'2026-09-08T15:00:00Z'}]}:job)
    .map(job=>scenario==='parked-location'?{...job,status:'Completed',truck:'Truck 9',customerName:'Parked report customer',address:'1701 Example Boulevard, New Orleans, LA 70114',location:{latitude:29.9358,longitude:-90.0282},truckAtJob:true,atJobTruck:'Truck 9',atJobGpsAt:new Date(Date.now()-30*60_000).toISOString(),onsiteTime:{minutes:null,arrival:null,departure:null,label:'Arrival and duration unconfirmed'}}:job)
    ...
    .map(job=>longDetails?{...job,address:'100 Example Boulevard, Building Three, Second Floor, Suite 237, New Orleans, LA 70130',junkItems:['Sofa, mattress, bookcases, and boxes stored in the upstairs room; use the side entrance.'],phone:'(555) 010-1001',appointmentNotes:['Use the side entrance. Call before arrival.','Additional source history remains in full details.'],photos:[{url:'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22400%22 height=%22300%22%3E%3Crect width=%22400%22 height=%22300%22 fill=%22%236c7f8c%22/%3E%3C/svg%3E',category:'Before',fileName:'synthetic-before.svg'},{url:'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22400%22 height=%22300%22%3E%3Crect width=%22400%22 height=%22300%22 fill=%22%232b6f54%22/%3E%3C/svg%3E',category:'After',fileName:'synthetic-after.svg'}],location:routeMode==='address'?null:{latitude:30,longitude:-90}}:job);
}
window.fetch=async(input,init)=>{
  ...
  if(url.pathname==='/api/desktop/schedule')return Response.json({date,observedAt:'2026-09-07T21:00:00Z',sourceRequest:{state:'ready',message:''},assignmentRecoveryNotices:scenario==='auto-recovery' && Date.now()-recoveryStartedAt>=20000?[{requestId:'synthetic-recovery',recordId:date+':appointment:1001',message:recoveryMessage}]:[],appointments:appointments(date),truckLoads:new URLSearchParams(location.search).has('loads')?[{truck:'Truck 1',label:'Full truck',note:'Synthetic load'},{truck:'Truck 3',label:'Full + 1/6',note:'Synthetic load'}]:undefined,fleet:{isToday:progressEnabled || scenario==='on-site' || scenario==='parked-location' || scenario==='truck-scroll' || scenario==='co-located' || longDetails && date==='2026-09-07',lastUpdatedAt:null,trucks:progressEnabled?[{truck:'Truck 9',latitude:30.45,longitude:-91.18,lastGpsUpdate:progressGps(),speed:progressStage.startsWith('parked')?0:25,ignition:progressStage.startsWith('parked')?'OFF':'ON',freshnessLabel:'Recent GPS',operationalStatus:'Available',driver:'Example driver',navigator:'',serviceStatus:'Available'}]:scenario==='on-site'?[{truck:'Truck 8',latitude:30.45,longitude:-91.18,lastGpsUpdate:new Date(Date.now()-300_000).toISOString(),freshnessLabel:'GPS Stale',operationalStatus:'GPS Stale',ignition:'OFF',driver:'Example Driver',navigator:'Example Navigator',serviceStatus:'Unavailable'}]:scenario==='parked-location'?[{truck:'Truck 9',latitude:29.9358,longitude:-90.0282,lastGpsUpdate:new Date(Date.now()-30*60_000).toISOString(),speed:0,ignition:'OFF',freshnessLabel:'Parked report',operationalStatus:'Parked',driver:'Example driver',navigator:'',serviceStatus:'Available'}]:scenario==='co-located'?[{truck:'Truck 3',latitude:30.40001,longitude:-91.16001,lastGpsUpdate:new Date(Date.now()-(stalePresence?80:30)*60_000).toISOString(),speed:0,ignition:'OFF',freshnessLabel:'Parked report',operationalStatus:'Parked',driver:'',navigator:'',serviceStatus:'Available'},{truck:'Truck 6',latitude:30.40006,longitude:-91.16008,lastGpsUpdate:new Date(Date.now()-20*60_000).toISOString(),speed:0,ignition:'OFF',freshnessLabel:'Parked report',operationalStatus:'Parked',driver:'',navigator:'',serviceStatus:'Available'}]:scenario==='truck-scroll'?[{truck:'Truck 3',latitude:30.45,longitude:-91.18,lastGpsUpdate:new Date().toISOString(),speed:0,ignition:'ON',freshnessLabel:'Recent GPS',operationalStatus:'Available',driver:'Example Driver',navigator:'Example Navigator',serviceStatus:'Available'},{truck:'Truck 4',latitude:30.46,longitude:-91.19,lastGpsUpdate:new Date().toISOString(),speed:0,ignition:'OFF',freshnessLabel:'Recent GPS',operationalStatus:'Available with an intentionally long status that wraps in the detail card',driver:'Example Driver With A Long Display Name',navigator:'Example Navigator With A Long Display Name',serviceStatus:'Service note with additional wrapping text'}]:[]}});
