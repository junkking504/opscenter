            ...
            <ol>{route.appointmentIds.map((id,index)=>{
              ...
              return <li key={id}>
                ...
                <div className="route-plan-stop"><span className={`route-plan-number territory-${region.code.toLowerCase()}`}>{index+1}</span>
                  <div className="route-plan-detail"><button className="route-plan-reference" disabled={busy} onClick={()=>select(id)}>{job.jkNumber||'JK Pending'} · {job.customerName||'Customer Unavailable'}</button><span>{job.appointmentTime||'Time Not Set'} · {region.label} · {job.appointmentType}</span><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(job.address)}`} target="_self" rel="noopener noreferrer">{job.address||'Address Unavailable'}</a><small>{valid&&stop?`${index===0?'Planned Start':'Planned Arrival'}: ${time(stop.arrival)}${stop.warnings.length?` · ${stop.warnings.join(' · ')}`:''}`:'Estimates Need Recalculation'}</small></div>
