...
import type { CrewPhone, CrewPhoneDirectory } from '@/lib/crew-phone';
...
export default function CompanyPhones() {
  ...
  return <main className={styles.page}><div className={styles.content}>
    ...
    {enrollment && <section className={styles.card} aria-live="polite"><h2>{enrollment.label}</h2><p>{enrollment.truck}</p><p>On the company phone, open <strong>/crew-jobs</strong> at this OpsCenter address and enter this 6-digit code:</p><p className={styles.code}>{enrollment.code}</p><p>Expires {new Date(enrollment.expiresAt).toLocaleTimeString([], {hour:'numeric', minute:'2-digit'})}. Usable by one phone.</p><button className={styles.secondary} disabled={busy} onClick={() => void send({ action: 'revoke', deviceId: enrollment.deviceId })}>Cancel this setup code</button></section>}
