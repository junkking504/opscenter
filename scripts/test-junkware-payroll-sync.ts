...
import {verifiedJunkwareShiftPay} from '../lib/corrected-crew-pay';
...
async function main(){try{
 ...
 const wageInput={amounts:{hours:7.33,regularHours:5.25,overtimeHours:2.08,jobs:7,revenue:100,labor:134,tips:36.99,bonuses:0,supplemental:0,totalPay:170.99},isSalary:false,date:'2026-09-06',clockIn:'10:40 AM',clockOut:'06:00 PM',hourlyRate:16,latestCorrectionAt:Date.parse('2026-09-07T22:22:00Z'),sync:{status:'verified',verifiedAt:'2026-09-07T23:13:00Z',after:{...shift,clockIn:'10:40 AM',clockOut:'06:00 PM',hourlyRate:16,hours:7.33,regularHours:5.25,overtimeHours:2.08,labor:133.92}}};
 assert.equal(verifiedJunkwareShiftPay(wageInput)?.amounts.totalPay,170.91,'Use the verified source wage, preserving source tips');assert.equal(wageInput.amounts.labor,134);
 assert.equal(verifiedJunkwareShiftPay({...wageInput,latestCorrectionAt:Date.parse('2026-09-08T00:00:00Z')}),null,'An earlier-day correction invalidates old source overtime');
 assert.equal(verifiedJunkwareShiftPay({...wageInput,clockOut:'05:00 PM'}),null,'Verified source must match current shift fields');
 assert.equal(verifiedJunkwareShiftPay({...wageInput,isSalary:true}),null);
 assert.equal(verifiedJunkwareShiftPay({...wageInput,amounts:{...wageInput.amounts,tips:null}})?.amounts.totalPay,null);
