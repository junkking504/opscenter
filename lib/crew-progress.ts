...
export function buildCrewProgress(input: {
  ...
  const progressJobs = jobs.map(([id, job]) => {
      ...
      const step = (label: string, state: CrewStep['state'], detail: string) => steps.push({label,state,detail});
      ...
      if (canceled) {
        ...
        step('Assignment', truckKey(job.truck) ? 'complete' : awaiting(true), truckKey(job.truck) ? truckName(job.truck) : 'Assign a truck and confirm the crew.');
        step('Arrival', arrived ? 'complete' : !input.visitsCurrent ? 'unknown' : !closed && pastWindow ? 'missing' : closed ? 'unknown' : 'next',
          arrived ? 'Confirmed visit recorded.' : !input.visitsCurrent ? 'Visit source unavailable or stale.' : pastWindow ? 'No confirmed arrival recorded after the window; confirm crew status.' : 'Awaiting a confirmed arrival.');
        step('Photos', job.photos.length ? 'complete' : !job.photoAuditAvailable ? 'unknown' : awaiting(closed),
          job.photos.length ? `${job.photos.length} uploaded · photo coverage still requires review.` : !job.photoAuditAvailable ? 'Photo collection has not been verified.' : closed ? 'No uploaded photos found for this closed appointment.' : 'Upload the required job photos.');
        ...
        step('Payment', estimate ? 'not-required' : paymentComplete ? 'complete' : awaiting(closed),
          estimate ? 'Payment is not required for an estimate.' : paymentComplete ? 'Payment recorded in JunkWare; settlement verification is separate.' : typeof balance === 'number' && balance > .01 ? `Recorded balance: $${balance.toFixed(2)}. Confirm remaining payment.` : 'Confirm payment details or document why no payment is due.');
        step('Closeout', closed ? 'complete' : awaiting(departed), closed ? `${estimate ? 'Estimate' : 'Job'} closed in JunkWare.` : departed ? 'Departure recorded; the appointment remains open.' : 'Complete the appointment closeout.');
        step('Departure', departed ? 'complete' : !input.visitsCurrent ? 'unknown' : 'next', departed ? 'Confirmed departure recorded.' : !input.visitsCurrent ? 'Visit source unavailable or stale.' : 'Awaiting a recorded departure.');
      }
      ...
      return {
        ...
        territory:job.territory || 'Territory unavailable', window:job.appointmentTime || 'Time not recorded', status:job.status || 'Status unavailable',
        href:desktopAppointmentHref(job.jkNumber,input.date), steps,
        next:canceled ? 'Review cancellation and the next stop.' : nextStep?.detail || 'All tracked steps recorded. Review photo coverage and payment verification.',
