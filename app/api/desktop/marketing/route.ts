...
import { readDesktopMarketing, updateDesktopMarketing } from '@/lib/desktop-marketing';
...
export async function GET(request: Request) {
  ...
  if (!opsRoleCan(actor.role, 'operations.read')) return Response.json({ error: 'Manager access is required.' }, { status: 403, headers });
  ...
  try {
    ...
    return Response.json(readDesktopMarketing(date, actor.role), { headers });
  } catch { return Response.json({ error: 'Marketing source unavailable.' }, { status: 503, headers }); }
}
export async function POST(request: Request) {
  const actor = await session();
  if (!actor) return Response.json({ error: 'Authentication required.' }, { status: 401, headers });
  if (!opsRoleCan(actor.role, 'operations.write')) return Response.json({ error: 'This role cannot change these records.' }, { status: 403, headers });
  ...
  try {
    ...
    if (operation.action === "review.assign" && !opsRoleCan(actor.role, "sensitive.write")) return Response.json({ error: "Manager access is required." }, { status: 403, headers });
    const receipt = updateDesktopMarketing(operation, actor);
