...
/** The Westbank is a visual Dispatch area within the New Orleans market. */
export function isWestbankLocation(...locationValues: unknown[]): boolean {
  return WESTBANK_LOCATION.test(locationValues.map((value) => String(value || "").trim()).filter(Boolean).join(" "));
}

export function isJeffersonCoreLocation(...locationValues: unknown[]): boolean {
  return JEFFERSON_CORE_LOCATION.test(locationValues.map((value) => String(value || "").trim()).filter(Boolean).join(" "));
}
