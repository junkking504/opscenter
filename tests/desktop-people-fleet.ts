...
import { readDesktopLocalReceipt, desktopVersion, executeDesktopLocalAction, readDesktopKrewe, runDesktopKreweAction } from '../lib/desktop-krewe';
import { displayedGpsFreshness, nullableNumber, sumObserved, validDesktopDate } from '../desktop-ui/lib/people-fleet-contract';
...
try {
 const now=Date.now(), today=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Chicago'}).format(now);
 assert.equal(displayedGpsFreshness(new Date(now-181000).toISOString(),today,now),'GPS Stale');
 assert.equal(displayedGpsFreshness(null,today,now),'GPS unavailable');
 ...
 let state:unknown=null;let executions=0;const input={requestId:randomUUID(),action:'fleet.issue',entity:'test:synthetic',expectedVersion:desktopVersion(null),values:{amount:5}};
 ...
 const receiptStatus=readDesktopLocalReceipt(input.requestId,'test@example.invalid','fleet');
 assert.equal(receiptStatus?.status,'verified');assert.equal('input' in receiptStatus!,false);
 assert.equal(readDesktopLocalReceipt(input.requestId,'other@example.invalid','fleet'),null);
 assert.equal(readDesktopLocalReceipt(input.requestId,'test@example.invalid','krewe'),null);
