      ...
      {formOpen ? (
        <div className="ops-repair-form">
          ...
          <div className="ops-repair-attachments">
            ...
            {(editingIssue?.attachments || []).map((attachment) => <div className="ops-repair-attachment" key={attachment.attachmentId}><a href={`/api/fleet-issue-attachments?attachmentId=${encodeURIComponent(attachment.attachmentId)}`} target="_self" rel="noreferrer">{attachment.fileName}</a><button type="button" onClick={() => removeAttachment(attachment)}>Remove</button></div>)}
