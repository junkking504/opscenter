...
export default function DailyCrew({date,roster,day,onSaved,onCancel,onBusy}:{date:string;roster:string[];day:CrewPhoneDay|null;onSaved:()=>void;onCancel:()=>void;onBusy:(busy:boolean)=>void}) {
 ...
 async function verify(){const response=await fetch('/api/crew-jobs/day',{cache:'no-store'});const body=await response.json();if(!response.ok)throw new Error(body.error || 'Today’s crew could not be verified.');if(body.day && body.date===date){onSaved();return true;}return false;}
 async function save(event:React.FormEvent){event.preventDefault();if(inFlight.current)return;inFlight.current=true;setBusy(true);onBusy(true);setError('');
  try{
   const input={date,expectedVersion:day?.version || 0,responsible,driver,navigators:navigator?[navigator]:[]};const fingerprint=JSON.stringify(input);
 ...
 return <section className={styles.card}><h1>{day?'Change today’s crew':'Enable phone for today'}</h1><p>{date} · Central time</p><p>Select who has this phone and who is working on the truck. These assignments fill each job’s closeout.</p>
  <form className={styles.form} onSubmit={save}>
   ...
   <button className={styles.primary} disabled={busy || !responsible || !driver || driver===navigator}>{busy?'Saving crew…':day?'Save today’s crew':'Enable phone for today'}</button>
  </form>
  ...
  {day && <button className={styles.secondary} disabled={busy} onClick={onCancel}>Keep current crew</button>}
 </section>;
}
