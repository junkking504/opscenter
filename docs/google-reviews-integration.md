...
The Reviews tab is a daily operational view: it shows the review details
returned by the current daily collection for every territory, sorted by Google
publish timestamp from newest to oldest. Older collection snapshots remain in
private history for change tracking, but are not rendered on that page.
