...
export function rescheduleTarget(job: DesktopAppointment, date: string, values: Record<string, unknown>, restore = false) {
  ...
  if (!restore && values.destinationDate === date && start === job.appointmentStartMinutes) throw new Error('A different appointment date or time is required.');
  ...
  return { appointmentId: job.appointmentId, date: values.destinationDate, appointmentStartMinutes: start, appointmentEndMinutes: start + duration, truck: match ? `Truck ${match[1]}` : '', ...(restore ? {status:'Confirmed'} : {}) };
}

export async function rescheduleAppointment(job: DesktopAppointment, date: string, values: Record<string, unknown>, restore = false) {
  const expected = rescheduleTarget(job, date, values, restore);
  return withJunkwareAppointmentSyncLock(job.appointmentId, async () => {
    try {
      const { stdout } = await promisify(execFile)(process.execPath, ['--import','tsx',path.join(process.cwd(),'scripts/reschedule-junkware-appointment.ts'), ...(restore ? ['--restore'] : []), '--appointment',job.appointmentId,'--date',expected.date,'--start-minutes',String(expected.appointmentStartMinutes),'--expected-date',date,'--expected-start-minutes',String(job.appointmentStartMinutes),'--expected-end-minutes',String(job.appointmentEndMinutes),'--expected-truck',expected.truck || 'unassigned'], {cwd:process.cwd(),timeout:180_000,maxBuffer:1024*1024,env:{...process.env}});
      const result = JSON.parse(stdout.trim());
      const matches = result.ok && result.mode === (restore ? 'restore' : 'reschedule') && result.appointmentId === expected.appointmentId && result.date === expected.date && result.appointmentStartMinutes === expected.appointmentStartMinutes && result.appointmentEndMinutes === expected.appointmentEndMinutes && result.truck === expected.truck && (!restore || result.status === 'Confirmed') && Number.isFinite(Date.parse(result.verifiedAt));
