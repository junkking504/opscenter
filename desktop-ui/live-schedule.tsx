...
import { needsScheduleAddressVerification, scheduleTruckNames, resolveScheduleDeepLink, scheduleMatchesQuery, scheduleStatusTone, scheduleCustomerLabel, scheduleMoveRestriction, unavailableRoute, assignmentNeedsVerification, appointmentCategory, appointmentColorClass, appointmentRegion, appointmentStatus, isClosed, timelineWindow, timelinePlacement, timelineRange, territoryLabels, territoryOrder, truckLabel, type ScheduleAppointment, type ScheduleRouting, type ScheduleSnapshot } from './lib/schedule-contract';
    ...
    <div ref={boardLayoutRef} className={`schedule-board-layout${showMap ? ' map-open' : ''}`}>
      {showMap && <section ref={mapPanelRef} className={`schedule-map-panel${selectedTruck ? ' has-truck-card' : ''}`}>
        ...
        <aside className="schedule-map-controls"><label className="schedule-gps-picker">Truck trips<select aria-label="Truck trips" value={selectedTruck || ''} onChange={event=>event.target.value?selectTruck(event.target.value):setSelectedTruck(null)}><option value="">Select truck</option>{truckNames.map(truck=><option key={truck} value={truck}>{truckDisplayText(truck)}</option>)}</select></label>{!selectedTruck && !selected && <><div><span className="section-kicker">{snapshot.fleet.isToday ? 'Live Map' : 'Planning Map'}</span><h2>{snapshot.fleet.isToday ? 'Dispatch Positions' : 'Appointment Coverage'}</h2></div>
          ...
          {selectedTruck ? <section aria-label={truckDisplayText(`${selectedTruck} appointments`)}>
      ...
      {!mapOnly && <div className="schedule-board-shell"><div className="section-title"><div><span className="section-kicker">{date} · JunkWare Snapshot</span><h2>Truck Schedule</h2><small className="schedule-time-basis" title="Booked blocks show the current JunkWare assignment. Recorded GPS visits remain read-only history on the truck that visited.">Blocks show booked assignments and recorded GPS visits</small></div><div className="schedule-board-actions"><ScheduleStopOrder key={`${date}:${stopOrderTruck}`} truck={stopOrderTruck} selectedAppointmentId={selectedId} snapshot={snapshot} busy={operationBusy || Boolean(pendingMove)} onBusyChange={onOperationBusyChange} saved={updated=>{setSnapshots(prior=>({...prior,[date]:updated}));refresh();}} /><span className="schedule-drag-help"><GripVertical size={13} />Drag Booked Appointment → Truck + Time</span></div></div>
        <div className="schedule-board-scroll"><div className={`schedule-board ${truckNames.length >= 10 ? 'ultra' : truckNames.length >= 7 ? 'compact' : 'comfortable'}`} style={{ '--schedule-hour-count': ticks.length } as CSSProperties}>
          ...
          {truckNames.map((truck, index) => {
            const rowJobs = jobs.filter(job => timelineWindow(job,truck,now.getTime()) !== null).sort((a, b) => (a.appointmentStartMinutes ?? Infinity) - (b.appointmentStartMinutes ?? Infinity));
