...
async function main(){
  ...
  assert.equal(href.searchParams.get('date'),'2026-09-01');assert.equal(href.searchParams.get('q'),'JK1234567');assert.equal(href.searchParams.has('appointmentId'),false);
  const choices=[{recordId:'one',appointmentId:'1',jkNumber:'JK1234567',customerName:'Test'},{recordId:'two',appointmentId:'2',jkNumber:'JK1234567',customerName:'Test'}] as ScheduleAppointment[];
  const resolved=resolveScheduleDeepLink(choices,href.searchParams.get('q')||'',href.searchParams.get('appointment')||'');
  assert.equal(resolved.query,'JK1234567');assert.equal(resolved.recordId,null);assert.match(resolved.notice,/Several appointments/);
