...
export default function DaySummary({summary:s}:{summary:WaypointDaySummary}){
 ...
 return <section aria-label="Today’s performance" className={styles.card}>
  ...
  {s.completed===null?<p>Completed appointments are unavailable.</p>:s.completed.length?<ul className={styles.list}>{s.completed.map(job=><li key={job.id}><strong>{job.customer}</strong><span>{job.reference} · {job.type}{job.time?` · ${job.time}`:''}</span><AddressLink address={job.address}/><span>{money(job.revenue)} revenue · {money(job.tips)} tips</span></li>)}</ul>:<p>No completed appointments for this truck yet.</p>}
 </section>;
}
