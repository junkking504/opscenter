...
## 2026-08-24 — Immutable-release collector integrity and refresh health

- Severity: critical (a pushed collector wrapper referenced an untracked helper, so the immutable release passed its login-only gate while every JunkWare refresh exited before collection).
- Root cause: deployment intentionally excludes dirty/untracked checkout files, but the deploy gate only requested `/login`; it neither resolved collector helper references against the built release nor observed the independent refresh loop.
- Fix: `deploy-release.sh` now mechanically verifies release-rooted script references from the release collector wrappers and live OpsBot refresh entrypoint before activation. Production then requires a full JunkWare refresh log sequence tied to the new release commit; failure uses the existing rollback path. The committed collector guard writes sanitized durable failures for DNS and non-network exits, with five-consecutive-cycle `@here` escalation through `#ops-data-health` and recovery on success.
- Regression coverage: `verify:collector-deploy` proves missing helper references fail preflight and that a release-specific full-refresh marker is required; `verify:data-collection-hardening` proves sanitization, success clearing, and fifth-failure escalation state; `verify:collector-alerting` proves an immediate incident, a fifth-cycle `@here`, and recovery delivery.
- Verification: lint and production build passed. Full `verify:*` result: 28/36 passed. Existing findings remain: missing August monthly reconciliation; stale appointment-block, WhatsApp-closeout, Slack-closeout, and UI-copy expectations; shared-CSS budget overage; missing isolated accessibility credentials; and Truck 9 live camera unavailable.
- Operational boundary: the deploy check waits for the existing independent collector; it does not start a duplicate refresh or restart collectors, tunnels, databases, or the WhatsApp worker.
