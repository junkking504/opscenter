...
export default function AppointmentCreation({ date, appointments, close, saved, onBusyChange }: { date: string; appointments: ScheduleAppointment[]; close: () => void; saved: () => void; onBusyChange: (busy: boolean) => void }) {
  ...
  const update = <K extends keyof typeof form>(key: K, value: typeof form[K]) => { if (mode !== 'edit'||pending.current) return; setForm(current => ({ ...current, [key]: value })); setDuplicateCheck(null);setCreateSeparate(false);setError(''); };
  ...
  return <><button className="record-drawer-backdrop" aria-label="Close new appointment" disabled={busy} onClick={close} /><aside ref={panel} className="record-drawer appointment-create-drawer" role="dialog" aria-modal="true" aria-labelledby="new-appointment-title" onKeyDown={event => { if (event.key !== 'Tab') return; const elements = [...event.currentTarget.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), select:not(:disabled), textarea:not(:disabled), a[href]')]; const first = elements[0], last = elements.at(-1); if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last?.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first?.focus(); } }}>
    ...
    <fieldset disabled={busy} className="record-drawer-body appointment-create-body">
      ...
      {mode === 'review' && duplicateCheck && <section className="prebooking-check" aria-label="Check Existing Appointments">
        ...
        {duplicateCheck.matches.map(job=><article key={job.recordId}><div><strong>{job.jkNumber||'JK Unavailable'}</strong><span>{appointmentCategory(job)} · {appointmentStatus(job)}</span></div><strong>{job.customerName}</strong><span> · {job.phone||'Phone Unavailable'}</span><p><a href={'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(serviceAddressForGeocoding(job.address))} target="_blank" rel="noopener noreferrer">{job.address}</a></p><p>{job.appointmentTime} · {job.truck} · {job.sourceTerritory||job.territory}</p><a href={'/desktop?workspace=Schedule&scheduleView=board&date='+encodeURIComponent(form.date)+'&appointment='+encodeURIComponent(job.recordId)} target="_blank" rel="noopener noreferrer">Open Existing Appointment ↗</a>{job.appointmentUrl&&/^https:\/\/junkware\.junk-king\.com\//i.test(job.appointmentUrl)&&<a href={job.appointmentUrl} target="_blank" rel="noopener noreferrer">Review in JunkWare ↗</a>}</article>)}
