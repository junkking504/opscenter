...
import { KNOWLEDGE_KINDS, KNOWLEDGE_WORKSPACES, knowledgeStatus, searchKnowledge, type KnowledgeAction, type KnowledgeDraft, type KnowledgeEntry, type KnowledgeSnapshot } from './lib/knowledge-contract';
import { workspaceLabel } from './lib/workspace-labels';
import './second-brain.css';

const kindLabel = { procedure: 'Procedure', decision: 'Decision', incident: 'Resolved incident' };
...
export default function SecondBrain({ workspace, disabled, navigate }: { workspace: string; disabled: boolean; navigate: (workspace: string) => void }) {
  ...
  function edit(value?: KnowledgeEntry) {
    ...
    setDraft(value ? { title: value.title, summary: value.summary, body: value.body, kind: value.kind, workspace: value.workspace,
      sourceLabel: value.sourceLabel, sourceUrl: value.sourceUrl, sourceNote: value.sourceNote, owner: value.owner } : blank(workspace));
  ...
  const rows = searchKnowledge(snapshot?.entries || [], query, scope, kind, status);
  ...
  return <>
    <button className="brain-launch" disabled={disabled} onClick={() => { setScope(workspace); setOpen(true); link(selectedId || '1'); }}><BookOpen size={17} /><span>Second Brain</span></button>
    {open && <dialog ref={dialog} className="brain-dialog" aria-labelledby="brain-title" onKeyDown={event => event.stopPropagation()} onCancel={event => { event.preventDefault(); close(); }}>
      <header className="brain-header"><div><p className="brain-eyebrow">OPSCENTER KNOWLEDGE</p><h2 id="brain-title">Second Brain</h2><p>Procedures, decisions, and lessons worth keeping.</p></div><button className="brain-icon" aria-label="Close Second Brain" disabled={locked} onClick={() => close()}><X size={22} /></button></header>
      <div className="brain-body" ref={content}>
        ...
        {draft ? <form className="brain-editor" onSubmit={event => { event.preventDefault(); void submit({ action: 'save', id: editId!, expectedVersion: editVersion, requestId: crypto.randomUUID(), draft }); }}>
          ...
          <div className="brain-toolbar"><label className="brain-search">Search knowledge<input type="search" placeholder="Try closeout, photos, or source…" value={query} maxLength={200} onChange={e => setQuery(e.target.value)} /></label>{canWrite && <button className="brain-primary" onClick={() => edit()}><Plus size={16} />Capture knowledge</button>}</div>
          <div className="brain-fields brain-filters"><label>Workspace<select value={scope} onChange={e => setScope(e.target.value)}>{KNOWLEDGE_WORKSPACES.map(value => <option key={value} value={value}>{value === 'All' ? 'All workspaces' : workspaceLabel(value)}</option>)}</select></label><label>Type<select value={kind} onChange={e => setKind(e.target.value)}><option value="all">All types</option>{KNOWLEDGE_KINDS.map(value => <option key={value} value={value}>{kindLabel[value]}</option>)}</select></label><label>Status<select value={status} onChange={e => setStatus(e.target.value)}><option value="all">Active entries</option><option value="review">Needs review / review due</option><option value="verified">Verified</option><option value="documented">Documented guides</option><option value="archived">Archived</option></select></label></div>
          {!snapshot ? <p role="status">{error ? 'Knowledge could not be loaded.' : 'Loading knowledge…'}</p> : <><p className="brain-count" role="status">{rows.length} {rows.length === 1 ? 'entry' : 'entries'}{scope !== 'All' ? ` relevant to ${workspaceLabel(scope)}` : ' across OpsCenter'} · Search includes steps, source references, and owners.</p>
            <div className="brain-results">{rows.map(item => <button className="brain-card" key={item.id} onClick={() => select(item.id)}><div className="brain-tags"><span>{kindLabel[item.kind]}</span><span>{workspaceLabel(item.workspace)}</span><span className={`brain-status brain-status-${item.status}`}>{knowledgeStatus(item)}</span></div><h3>{item.title}</h3><p>{item.summary}</p><small>{item.sourceLabel} · {date(item.updatedAt)}</small></button>)}</div>
            {!rows.length && <div className="brain-empty"><h3>No entries match these filters.</h3><p>Try another phrase or look across all workspaces.</p><button onClick={() => { setQuery(''); setScope('All'); setKind('all'); setStatus('all'); }}>Reset filters</button></div>}
