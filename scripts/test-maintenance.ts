...
async function main() {
  ...
  await diagnoseMaintenance(ai, 'test-key', () => { saves++; }, now + 60_000, mock, () => true);
  assert.equal(ai.incidents[0].diagnosis?.summary, diagnosis.summary);
  assert.equal(ai.months[monthKey(now)].committedMicros, 440);
  await diagnoseMaintenance(ai, 'test-key', () => {}, now + 3_600_000, mock, () => true);
  ...
  await diagnoseMaintenance(failure, 'test-key', () => {}, now + 60_000, unavailable, () => true);
  ...
  await diagnoseMaintenance(failure, 'test-key', () => {}, now + 120_000, unavailable, () => true);
  assert.equal(failures, 1, 'provider failures obey cooldown');
  await diagnoseMaintenance(failure, 'test-key', () => {}, now + 3_660_000, unavailable, () => true);
  await diagnoseMaintenance(failure, 'test-key', () => {}, now + 7_260_000, unavailable, () => true);
  await diagnoseMaintenance(failure, 'test-key', () => {}, now + 10_860_000, unavailable, () => true);
