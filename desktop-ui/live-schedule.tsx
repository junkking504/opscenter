...
import { needsScheduleAddressVerification, scheduleTruckNames, resolveScheduleDeepLink, scheduleMatchesQuery, scheduleStatusTone, scheduleCustomerLabel, scheduleMoveRestriction, unavailableRoute, assignmentNeedsVerification, appointmentCategory, appointmentColorClass, appointmentRegion, appointmentStatus, isClosed, timelinePlacement, timelineRange, territoryLabels, territoryOrder, truckLabel, type ScheduleAppointment, type ScheduleRouting, type ScheduleSnapshot } from './lib/schedule-contract';
...
export default function LiveSchedule({ baseDate, day, onDayChange, onCounts, report, view = 'board', onOpenDate, onBusyChange, mapOnly = false }: Props) {
  ...
  const [truckMapView, setTruckMapView] = useState<'location' | 'route' | 'overview'>('overview');
  ...
  useEffect(() => {
    ...
    // Record links are a one-time navigation intent, not a permanent filter.
    // Leaving them in the URL restores stale searches on remount/date changes.
    if (params.has('q') || params.has('appointment')) {
      const url = new URL(window.location.href);
      url.searchParams.delete('q'); url.searchParams.delete('appointment');
  ...
  const match = (job: ScheduleAppointment) => {
    ...
    return scopeMatch && scheduleMatchesQuery(job, searchQuery) && (filter === 'all' || filter === 'completed' && appointmentStatus(job) === 'Completed' || filter === 'estimates' && appointmentStatus(job) === 'Estimate Closed' || filter === 'open' && !isClosed(job) || filter === 'unassigned' && truckLabel(job.truck) === 'Unassigned' || filter === 'verify' && needsScheduleAddressVerification(job));
  };
  ...
  const selectAppointment = (id: string) => {
    ...
    setSelectedTruck(null); setSelectedId(id); setMapResetKey(key => key + 1); setShowMap(true);
    setDrawerId(view === 'board' ? null : id);
    // The timeline retains muted out-of-filter appointments. Selecting one
    // must also reveal its locator instead of focusing an absent marker.
  ...
  const stopOrderTruck = selectedTruck || (selected ? truckLabel(selected.truck) : null);
  ...
  return <TruckCameraController className="live-schedule-camera"><section className={`schedule-workspace live-schedule${mapOnly ? ' command-map-content' : ' schedule-dispatch'}`}>
    ...
    {view === 'board' && <>{!mapOnly && <div className="schedule-summary-strip">{[
      ['all', 'Scheduled', jobs.length], ['completed', 'Completed Jobs', jobs.filter(job => appointmentStatus(job) === 'Completed').length], ['estimates', 'Closed Estimates', jobs.filter(job => appointmentStatus(job) === 'Estimate Closed').length], ['open', 'Open', jobs.filter(job => !isClosed(job)).length], ['unassigned', 'Unassigned', jobs.filter(job => truckLabel(job.truck) === 'Unassigned').length], ['verify', 'Location pending', jobs.filter(needsScheduleAddressVerification).length],
    ...
    <div ref={boardLayoutRef} className={`schedule-board-layout${showMap ? ' map-open' : ''}`}>
      {showMap && <section ref={mapPanelRef} className={`schedule-map-panel${selectedTruck ? ' has-truck-card' : ''}`}>
        ...
        <div className="schedule-map-canvas">
          ...
          <div className="map-focus-chip"><span>{filtered ? 'Filtered View' : 'Operating Footprint'}</span><strong>{selected ? `${selected.jkNumber} · ${needsScheduleAddressVerification(selected) ? 'Location pending' : 'Selected'}` : selectedTruck || (scope === 'ALL' ? 'All Territories' : territoryLabels[scope.split(':')[0]] || scope)}</strong>{(filtered || selectedId || selectedTruck) && <div className="map-focus-actions"><button onClick={reset}>Reset</button></div>}</div>
      ...
      {!mapOnly && <div className="schedule-board-shell"><div className="section-title"><div><span className="section-kicker">{date} · JunkWare Snapshot</span><h2>Truck Schedule</h2><small className="schedule-time-basis" title="Confirmed arrival through departure; booked windows remain when visit timestamps are unavailable.">Completed blocks show time on site</small></div><div className="schedule-board-actions"><ScheduleStopOrder key={`${date}:${stopOrderTruck}`} truck={stopOrderTruck} selectedAppointmentId={selectedId} snapshot={snapshot} busy={operationBusy || Boolean(pendingMove)} onBusyChange={onOperationBusyChange} saved={updated=>{setSnapshots(prior=>({...prior,[date]:updated}));refresh();}} /><span className="schedule-drag-help"><GripVertical size={13} />Drag Appointment → Truck + Time</span></div></div>
        <div className="schedule-board-scroll"><div className={`schedule-board ${truckNames.length >= 10 ? 'ultra' : truckNames.length >= 7 ? 'compact' : 'comfortable'}`} style={{ '--schedule-hour-count': ticks.length } as CSSProperties}>
          ...
          {truckNames.map((truck, index) => {
            ...
            return <div className="schedule-truck-row" data-schedule-truck={truck} data-natural-height={rowHeight} key={truck} style={{ flex: `0 0 var(--schedule-row-height, ${rowHeight}px)`, '--schedule-row-min-height': `${rowHeight}px` } as CSSProperties}><button type="button" className="schedule-truck-cell" aria-label={`Select ${truck} on map`} aria-pressed={selectedTruck === truck} onClick={() => selectTruck(truck)}><i className={['blue', 'red', 'gold', 'purple'][index % 4]} /><strong>{truck}</strong><span>{rowJobs[0] ? crew(rowJobs[0]) : 'No Scheduled Work'}</span>{load && <small className={`schedule-truck-load${load.needsVerification || (load.percent ?? 0) > 100 ? ' warning' : ''}`} title={load.note}>{load.label}</small>}</button><div className="live-truck-timeline">
              ...
              <div className="schedule-timeline-content">{placed.flatMap(({ job, position, lane }) => position.segments.map((segment, segmentIndex) => <div key={`${job.recordId}:${segmentIndex}`} className={`schedule-appointment status-${scheduleStatusTone(job)} ${appointmentColorClass(job)} ${slug(appointmentCategory(job))} ${slug(appointmentStatus(job))}${filtered ? match(job) ? ' scope-match' : ' scope-muted' : ''}${selectedId === job.recordId ? ' route-selected' : ''}${drag.preview?.job.recordId === job.recordId ? ' is-dragging' : ''}`} style={{ left: `${segment.left * 100}%`, width: `max(2px, calc(${segment.width * 100}% - 2px))`, top: `${lane * laneStep + 2}px`, height: 22 }} role="button" aria-pressed={selectedId === job.recordId} data-schedule-appointment={job.recordId} data-time-basis={position.actual ? "actual" : "booked"} tabIndex={0} aria-roledescription={scheduleMoveRestriction(job) || operationBusy ? undefined : "draggable appointment"} onPointerDown={event => drag.begin(event, job)} aria-label={`${job.jkNumber} · ${job.customerName} · ${job.appointmentTime} booked · ${appointmentStatus(job)} · ${position.label}${appointmentPartner(job) ? ` · ${appointmentPartner(job)!.name}` : ''}`} aria-description={scheduleMoveRestriction(job) || "Drag to change truck or time; click to show the job summary and highlight its map location."} onClick={() => { if (!drag.suppressClick.current) selectAppointment(job.recordId); }} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectAppointment(job.recordId); } }} title={`${job.jkNumber} · ${job.customerName} · ${job.appointmentTime} booked · ${appointmentRegion(job).label} · ${appointmentStatus(job)} · ${position.label}${position.actual ? ` · ${clock(position.intervals[segmentIndex].start)}–${clock(position.intervals[segmentIndex].end)}` : ""}${appointmentPartner(job) ? ` · ${appointmentPartner(job)!.name}` : ''}${scheduleMoveRestriction(job) ? ` · ${scheduleMoveRestriction(job)}` : ""}`}>{appointmentPartner(job) && <span className="schedule-partner-cue" title={appointmentPartner(job)!.name} aria-hidden="true" />}{!scheduleMoveRestriction(job) && <GripVertical className="schedule-grip" size={9} aria-hidden="true" />}<em title={appointmentStatus(job)} className={`schedule-block-status status-${scheduleStatusTone(job)}`}>
    ...
    {!mapOnly && <><button type="button" className="schedule-appointments-jump" aria-controls="schedule-all-appointments" onClick={() => { reset(); requestAnimationFrame(() => { appointmentRegisterRef.current?.scrollIntoView({block:'start',behavior:'instant'}); appointmentRegisterRef.current?.focus({preventScroll:true}); }); }}><strong>All Appointments <span>{jobs.length}</span></strong><span>View list below <ArrowDown size={18} aria-hidden="true" /></span></button><div className="live-schedule-status">{snapshot.observedAt ? `JunkWare snapshot: ${new Date(snapshot.observedAt).toLocaleString('en-US', { timeZone: 'America/Chicago' })}` : 'JunkWare snapshot timestamp unavailable'} · {routeState || (displayLegs.length ? `${displayLegs.filter(leg => leg.travelMinutes !== null).length} of ${displayLegs.length} travel estimates available${routing?.calculatedAt ? ` · Calculated ${new Date(routing.calculatedAt).toLocaleTimeString('en-US', { timeZone: 'America/Chicago', hour: 'numeric', minute: '2-digit' })}` : ''}. OpenStreetMap road estimates · No live traffic.` : 'No consecutive assigned appointments to route.')} Appointment windows are not confirmed service durations.{jobs.some(job => !job.hasScheduledTime) && ` · ${jobs.filter(job => !job.hasScheduledTime).length} untimed appointments are listed below.`}</div>
    <section className="appointment-register" id="schedule-all-appointments" ref={appointmentRegisterRef} tabIndex={-1} aria-label="All Appointments"><div className="section-title appointment-register-title"><div><span className="section-kicker">{visible.length} Shown · {selectedTruck || (scope === 'ALL' ? 'All Territories' : territoryLabels[scope.split(':')[0]] || scope)}</span><h2>All Appointments</h2></div>{filtered && <Button variant="outline" size="sm" onClick={reset}>Show All {jobs.length}</Button>}</div>
      ...
      <div className="appointment-register-table" role="table" aria-label="All scheduled appointments">{[...groups].sort((a, b) => a.code === priority ? -1 : b.code === priority ? 1 : territoryOrder.indexOf(a.code) - territoryOrder.indexOf(b.code)).map(group => {
        ...
        return <section className={`appointment-territory-group ${slug(group.label)}`} role="rowgroup" key={group.code}><div className="appointment-territory-heading"><div><i /><button onClick={() => focusTerritory(group.code)}>{group.code}</button><button className="territory-record-link" onClick={() => focusTerritory(group.code)}>{group.label}<ArrowRight size={11} /></button></div><span>{groupJobs.length} Appointments</span></div><div className="appointment-register-head" role="row"><span>Appointment</span><span>Customer &amp; work</span><span>Assignment</span><span>Payment · JunkWare recorded</span><span>Status &amp; details</span></div>
          {[...new Set(groupJobs.map(region => region.areaCode))].map(areaCode => {
            const areaJobs = groupJobs.filter(region => region.areaCode === areaCode);
            return <section className={`appointment-area-group ${areaCode.toLowerCase()}`} key={areaCode}>
              <div className="appointment-area-heading"><div><button onClick={() => focusTerritory(`${group.code}:${areaCode}`)}>{areaCode}</button><button className="area-record-link" onClick={() => focusTerritory(`${group.code}:${areaCode}`)}>{areaJobs[0].area}<ArrowRight size={10} /></button></div><span>{areaJobs.length} Appointments</span></div>
              {areaJobs.map(({ job, area, mismatch, needsReview, reason, sourceTerritory }) => {
