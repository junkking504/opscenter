...
export default function MobileCloseoutPreview({ jobs, date }: { jobs: ScheduleAppointment[]; date: string }) {
  ...
  return <div className="crew-mobile ops-live">
    ...
    {!selected ? <main className="mobile-home">
      ...
      <h1 ref={heading} tabIndex={-1}>Let’s get it done.</h1><p className="mobile-subtitle">Your truck. Your jobs. One place to finish.</p>
      ...
      <div className="mobile-section-heading"><h2>Today’s jobs</h2><span>Truck 6</span></div>
      ...
      <main className="mobile-record-content">
        <section hidden={screen!=='job'}>
          ...
          <div className="mobile-info-card"><h2>What we’re picking up</h2><p>{selected.junkItems.join(', ')}</p></div>
        ...
        <section hidden={screen!=='photos'}><h2>Show the finished job.</h2><p className="mobile-subtitle">Add before and after photos while you’re on site.</p>
        ...
        <section hidden={screen!=='closeout'} className="job-record-drawer mobile-closeout-host">
          <div className="mobile-closeout-intro"><h2>Finish this job</h2><p>Check the details, then review before saving.</p></div>
