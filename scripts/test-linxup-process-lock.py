#!/usr/bin/python3
"""Synthetic process lifecycle tests; no GPS, network, credentials or alerts."""
...
module_path = Path(__file__).with_name('run-linxup-locked.py').resolve()
spec = importlib.util.spec_from_file_location('gps_lock', module_path)
...
class LockTest(unittest.TestCase):
    ...
    def test_reuse_and_stable_inode(self):
        ...
        inode = (self.lock / 'worker.lock').stat().st_ino
        self.assertEqual(self.run_job(), 0)
        self.assertEqual((self.lock / 'worker.lock').stat().st_ino, inode)

    def test_legacy_owner_is_not_stolen(self):
        self.lock.mkdir()
        self.assertEqual(self.run_job(busy_code=75), 75)
    ...
    def test_launcher_crash_retains_live_child_lock(self):
        ...
        try:
            ...
            launcher.kill()
            launcher.wait()
            self.assertEqual(self.run_job(busy_code=75), 75, 'A live child must still exclude concurrent writers')
            time.sleep(1.6)
