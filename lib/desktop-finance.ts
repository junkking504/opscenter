...
import { recordReviewedRecyclingReceipt } from './recycling-receipt-review';
import { readFinancialStatements } from "./financial-statements";
...
import { buildDailyFinanceSummary, type DailyFinanceSummary } from '@/lib/daily-finance-summary';
...
export function readDesktopFinance(date: string): FinanceData & { fuelReconciliation: ReturnType<typeof readFuelReconciliation>; dailyBreakdown: DailyFinanceSummary; wexFuel: ReturnType<typeof readWexFuelFinance>; dumpExpenses: ReturnType<typeof readDumpExpenses> } {
  ...
  const dailySummary = { revenue: dailyBreakdown.revenue, costs: dailyBreakdown.totalCosts, profit: dailyBreakdown.net, recyclingIncome: finite(metrics?.recycling_income ?? metrics?.truck_record_financial_summary?.recycling_income) };

  return { fuelReconciliation, dumpExpenses: readDumpExpenses(date), dailyBreakdown, recyclingReceipts: readRecyclingData().receiptDrafts || [], statements: readFinancialStatements(), wexFuel, comparison:financePeriodComparison(monthly.range.dataThroughDate,readDaily), date, available: Boolean(metrics), generatedAt: metrics?.generated_at || metrics?.updated_at || null,
    daily: dailySummary,
...
export function updateDesktopFinance(operation: CommercialOperation, actor: { email: string; role: InteractiveOpsRole }) {
  ...
  const runDate = values.runDate === undefined ? undefined : text(values.runDate);
  const paymentDate = values.paymentDate === undefined ? undefined : text(values.paymentDate);
  if (runDate !== undefined && !validCommercialDate(runDate)) throw new CommercialActionError('Enter a valid run date.');
  if (paymentDate && !validCommercialDate(paymentDate)) throw new CommercialActionError('Enter a valid payment date.');
  if (status === 'Paid' && paymentDate === '') throw new CommercialActionError('Enter the payment received date.');
  ...
  return executeCommercialOperation(operation, actor, () => { const records = readRecycling(); return records.find(record => record.id === operation.recordId)?.version || commercialVersion(records); }, () => {
    const store = readRecyclingStore(); const records = store.records; const existing = records.find(item => item.id === operation.recordId); const value = { ...existing, id: operation.recordId, date: runDate ?? existing?.date ?? operation.date, paymentDate: paymentDate ?? existing?.paymentDate, material, sourceJob, yard, quantity, owner, ticket, paymentReference, note, status, expectedValue, realizedValue, updatedAt: new Date().toISOString() };
    const record = { ...value, version: commercialVersion(value) };
    const next = records.filter(item => item.id !== record.id).concat(record);
    const file = recyclingFile(); fs.writeFileSync(`${file}.tmp`, JSON.stringify({ ...store, records: next }), { mode: 0o600 }); fs.renameSync(`${file}.tmp`, file);
