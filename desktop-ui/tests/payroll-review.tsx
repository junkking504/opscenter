...
function Fixture(){
 ...
 return <main style={{padding:20,maxWidth:1400,margin:'auto'}}><h1>Synthetic payroll review</h1><p>All records are fictional. This fixture makes no source requests.</p><button onClick={()=>setChanged(value=>!value)}>Change source bonus</button> · <button onClick={()=>setFailed(value=>!value)}>Toggle refresh failure</button><p>{sourceOpen}</p><LiveKreweHours date={date} payroll={payroll} sourceError={failed?'Synthetic refresh failure':''} onRefresh={async()=>{if(failed)throw new Error('Synthetic refresh failure');setSourceOpen('Snapshot refreshed');}}/></main>;
}
