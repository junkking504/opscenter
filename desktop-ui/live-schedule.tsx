    ...
    <div ref={boardLayoutRef} className={`schedule-board-layout${showMap ? ' map-open' : ''}`}>
      {showMap && <section ref={mapPanelRef} className={`schedule-map-panel${selectedTruck ? ' has-truck-card' : ''}`}>
        ...
        <div className="schedule-map-canvas">
          ...
          <div className="map-focus-chip"><span>{filtered ? 'Filtered View' : 'Operating Footprint'}</span><strong>{selected ? `${selected.jkNumber} · ${selected.location ? 'Selected' : 'Verify Address'}` : selectedTruck || (scope === 'ALL' ? 'All Territories' : territoryLabels[scope.split(':')[0]] || scope)}</strong>{(filtered || selectedId || selectedTruck) && <div className="map-focus-actions">{selected && <button className="dispatch-full-details" aria-label={`Full details for ${selected.jkNumber} from map`} disabled={operationBusy} onClick={() => setDrawerId(selected.recordId)}>Full details <ArrowRight size={14} /></button>}<button onClick={reset}>Reset</button></div>}</div>
        ...
        <aside className="schedule-map-controls"><label className="schedule-gps-picker">Truck GPS route<select aria-label="Truck GPS route" value={selectedTruck || ''} onChange={event=>event.target.value?selectTruck(event.target.value):setSelectedTruck(null)}><option value="">Select truck</option>{truckNames.map(truck=><option key={truck} value={truck}>{truck}</option>)}</select></label>{!selectedTruck && !selected && <><div><span className="section-kicker">{snapshot.fleet.isToday ? 'Live Map' : 'Planning Map'}</span><h2>{snapshot.fleet.isToday ? 'Dispatch Positions' : 'Appointment Coverage'}</h2></div>
          ...
          {selectedTruck ? <section aria-label={`${selectedTruck} appointments`}>
            ...
            <header><div><span>Selected Appointment</span><strong>{selected.jkNumber} · {appointmentRegion(selected).area}</strong></div><div><button className="dispatch-full-details" disabled={operationBusy} onClick={() => setDrawerId(selected.recordId)}>Full details <ArrowRight size={14} /></button><button aria-label="Clear appointment selection" onClick={() => setSelectedId(null)}>×</button></div></header>
            ...
            <footer>Road estimates · No live traffic. Distance ranking only; availability, load, and schedule must be checked before assigning.</footer>
      ...
      {!mapOnly && <div className="schedule-board-shell"><div className="section-title"><div><span className="section-kicker">{day === 'today' ? 'Today' : 'Tomorrow'} · JunkWare Snapshot</span><h2>Truck Schedule</h2></div><div className="schedule-board-actions">{selected && <div className="schedule-selected-actions"><span>{selected.jkNumber} · {selected.customerName}</span><button className="dispatch-full-details" aria-label={`Full details for ${selected.jkNumber} from truck schedule`} disabled={operationBusy} onClick={() => setDrawerId(selected.recordId)}>Full details <ArrowRight size={14} /></button></div>}<ScheduleStopOrder key={date} snapshot={snapshot} busy={operationBusy || Boolean(pendingMove)} onBusyChange={onOperationBusyChange} saved={updated=>{setSnapshots(prior=>({...prior,[date]:updated}));refresh();}} /><span className="schedule-drag-help"><GripVertical size={13} />Drag Appointment → Truck + Time</span></div></div>
