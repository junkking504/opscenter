...
fail() {
  echo "MacBook preview deployment stopped: $*" >&2
  exit 1
}

usage() {
  echo "usage: $SCRIPT_NAME <mc-host-or-address> [git-ref]" >&2
  echo "   or: OPSCENTER_MC_HOST=<host> $SCRIPT_NAME [git-ref]" >&2
  exit 64
}
...
if [[ -n "$(git -C "$REPOSITORY_ROOT" status --short)" ]]; then
  echo "Note: uncommitted changes are not part of this preview deployment."
fi
