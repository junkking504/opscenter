...
async function main(){
 ...
 try{
  ...
  const date=new Date().toISOString().slice(0,10),id='900001',version='a'.repeat(64),current=releaseCrewJob({truck:phone.truck,date,appointmentId:id,expectedVersion:0,requestId:randomUUID()},'manager').current!;
  ...
  const baseline={truck:'Truck 6',status:{value:'1',label:'Confirmed'},paymentMethods:[{value:'cash',label:'Cash'},{value:'card',label:'Credit Card'},{value:'billed',label:'Billed'}],payments:[{description:'Cash',amount:'50.00'}],photoEvidence:{appointmentId:id,urls:[`https://junkware.junk-king.com/system/aspnet/local/media/photo-${id}-before.jpg`]}};
  ...
  const deps:NonNullable<Parameters<typeof submitCrewCloseout>[2]>={schedule:schedule as unknown as NonNullable<Parameters<typeof submitCrewCloseout>[2]>['schedule'],scope:(req,assignmentId,run)=>withCrewJob(req,assignmentId,run,scopeSources as never),read:async()=>{if(revokeOnRead)revokeCrewPhone(phone.deviceId,'manager');return{ok:true,appointmentId:id,closeout:source};},
   write:async()=>{writes++;if(lose)throw new JunkwareCloseoutError('Lost result','uncertain');source={...source,status:{value:'8'},payments:[...baseline.payments,{description:'Credit Card 1234',amount:'350.00'}]};return{ok:true,appointmentId:id,closeout:source,verifiedAt:new Date().toISOString()};},updateLoad:()=>({updated:true,status:null,reason:"Synthetic source"}),
  };
  const values={appointmentId:id,serviceDate:date,targetStatus:'8',truck:'Truck 6',appointmentType:'Job',expectedSourceVersion:closeoutSourceVersion(source),addPayment:{methodId:'card',amount:'350.00',reference:'1234'}};
  const body=()=>({assignmentId:current.assignmentId,requestId:randomUUID(),expectedVersion:version,values:{...values,expectedSourceVersion:closeoutSourceVersion(source)}});
