...
async function main(){
 ...
 try {for(const width of [320,390,430]){
  ...
  await page.route('**/api/**',async route=>{
   ...
   if(u.pathname==='/api/crew-jobs/current')return send({state:'assigned',truck:'Truck 6',job:completed?{assignmentId:'next-assignment',appointmentId:'900002',date:'2026-09-18',jkNumber:'SAMPLE-02',customerName:'Next Sample Customer',address:'200 Sample Street',appointmentTime:'12–2 PM',junkItems:['Second job'],appointmentNotes:[],driver:'Sample Driver',navigator:'Sample Navigator'}:{assignmentId:'sample-assignment',appointmentId:'900001',date:'2026-09-18',jkNumber:'SAMPLE-01',customerName:'Sample Customer',address:'100 Sample Street',appointmentTime:'10 AM–12 PM',junkItems:[],appointmentNotes:[],driver:'Sample Driver',navigator:'Sample Navigator'}});
  ...
  const open=async()=>{await page.getByRole('button',{name:'View assignment',exact:true}).click();await page.getByRole('button',{name:'Start closeout · Before photos',exact:true}).click();};
  ...
  await expect(page.getByRole('radio',{name:'Cancelled',exact:true})).toHaveCount(0);await page.getByRole('button',{name:'Continue to charges',exact:true}).click();await expect(page.getByLabel('Appointment truck')).toBeDisabled();await expect(page.getByRole('combobox',{name:'Driver',exact:true})).toBeDisabled();await expect(page.getByRole('combobox',{name:'Navigator 1',exact:true})).toBeDisabled();await page.getByRole('button',{name:'Continue to after photos',exact:true}).click();await page.getByRole('button',{name:'Continue to payment',exact:true}).click();
