...
function appointments(date:string):ScheduleAppointment[] {
  return Array.from({length:areaCases?.length ?? (scenario==='empty'?0:scenario==='crowded'?14:scenario==='dense'?24:scenario==='same-time'?4:['on-site','route-stack'].includes(scenario)?3:scenario==='truck-scroll'?2:1)},(_,index)=>{
    ...
    .map(job=>scenario==='co-located'?{...job,truck:'Truck 6',customerName:'Shared appointment',truckVisits:[{truck:'Truck 3',arrival:'2026-09-08T14:24:00Z',departure:'2026-09-08T15:10:00Z',observedThrough:'2026-09-08T15:10:00Z'},{truck:'Truck 6',arrival:'2026-09-08T14:34:00Z',departure:'2026-09-08T15:05:00Z',observedThrough:'2026-09-08T15:05:00Z'}]}:job)
