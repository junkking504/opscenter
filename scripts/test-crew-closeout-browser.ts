...
async function main(){
 ...
 try {for(const width of [320,390,430]){
  ...
  let posts=0,receipt:any=null,sourceVersion='a'.repeat(64),photos=true;
  const closeout=()=>({...fixture,photoEvidence:closeoutPhotoEvidence('900001',photos?['https://junkware.junk-king.com/system/aspnet/local/media/sample-900001-before.jpg']:[])});
  await page.route('**/api/**',async route=>{
   ...
   if(u.pathname==='/api/crew-jobs/day')return send({date:'2026-09-18',day:{date:'2026-09-18',version:1,responsible:'Sample Driver',driver:'Sample Driver',navigators:['Sample Navigator']},roster:['Sample Driver','Sample Navigator','Extra Crew']});
   if(u.pathname==='/api/crew-jobs/current')return send({state:'assigned',truck:'Truck 6',job:{assignmentId:'sample-assignment',appointmentId:'900001',date:'2026-09-18',jkNumber:'SAMPLE-01',customerName:'Sample Customer',address:'100 Sample Street',appointmentTime:'10 AM–12 PM',junkItems:[],appointmentNotes:[],driver:'Sample Driver',navigator:'Sample Navigator'}});
   ...
   if(u.pathname==='/api/crew-jobs/closeout'){
    if(r.method()==='POST'){
     ...
     receipt={requestId:b.requestId,action:'closeout',status:'verified',message:'Saved and verified in JunkWare.',sourceResult:{appointmentId:'900001',closeout:{...closeout(),status:{value:'8',label:'Completed'}}}};
     return route.abort();
    }
    if(u.searchParams.has('requestId'))return send({receipt});
  ...
  const open=async()=>{await page.getByRole('button',{name:'View job',exact:true}).click();await page.getByRole('button',{name:'Close out job',exact:true}).click();await expect(page.getByLabel('Appointment truck')).toBeDisabled();};
  await page.goto(`${process.argv[2] || 'http://127.0.0.1:3189'}/crew-jobs`);await open();
  await expect(page.getByRole('radio',{name:'Cancelled',exact:true})).toHaveCount(0);await expect(page.getByRole('combobox',{name:'Driver',exact:true})).toBeDisabled();await expect(page.getByRole('combobox',{name:'Navigator 1',exact:true})).toBeDisabled();
  await page.getByRole('button',{name:'Continue to charges',exact:true}).click();await page.getByRole('button',{name:'Continue to payment',exact:true}).click();
  ...
  sourceVersion='b'.repeat(64);await page.reload();await open();await page.getByRole('button',{name:'+ Add additional crew',exact:true}).click();await page.getByRole('combobox',{name:'Additional crew 1',exact:true}).selectOption('extra');await page.getByRole('button',{name:'Continue to charges',exact:true}).click();await page.getByRole('button',{name:'Continue to payment',exact:true}).click();await expect(page.getByLabel('Record a collected payment',{exact:true})).not.toBeChecked();
  ...
  await page.getByRole('button',{name:'Review Closeout',exact:true}).click();await expect(page.getByRole('button',{name:'Confirm Job Closeout in JunkWare',exact:true})).toBeVisible();
  ...
  await page.getByRole('button',{name:'Confirm Job Closeout in JunkWare',exact:true}).click();await expect(page.getByRole('heading',{name:'Closeout verified',exact:true})).toBeVisible();assert.equal(posts,1);
  await page.reload();await open();await expect(page.getByRole('button',{name:'Reload saved closeout',exact:true})).toBeVisible();assert.equal(posts,1);assert.deepEqual(errors,[]);
  await context.close();console.log(`PASS ${width}px: fixed truck, collected-card payment, draft refresh/source mismatch, review, lost response read-back, reload without payment replay, no overflow.`);
