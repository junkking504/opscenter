...
Average Job Size (AJS) in one compact desktop row of equal-width cards. All headline values share the same font size.
...
An unpinned operating day follows Chicago midnight after an active record/action closes. Explicit date selections remain pinned. All desktop workspaces share one absolute viewing date. Old Schedule links with a Tomorrow offset are normalized before loading data, and Schedule day controls change that shared date. The sticky Viewing day bar shows the weekday, date, year and Today/Historical/Upcoming context, with Previous, Next, Today and a date field plus Go. Changing the date preserves the current view and clears old appointment selection; opening a calendar day explicitly opens its board. Date changes remain blocked during an active operation. Monitor links include the operating date and available appointment, truck, or employee identity. The existing map remains in place with a shortcut to Needs Action. Regression coverage: `scripts/test-operating-day-navigation.ts`.
