...
export default function ScheduleAppointmentSummary({job,closest,loading,isToday,busy,open,clear}: {
  ...
  return <section className={`schedule-appointment-summary ${appointmentColorClass(job)}`} aria-label={`Selected job ${job.jkNumber}`}>
    ...
    <aside className="selected-job-next">
      ...
      {!isClosed(job) && <div className={`selected-job-closest${onsite ? '' : isToday && job.location && !loading && closest ? ' has-suggestion' : isToday && !job.location ? ' needs-review' : ''}`} aria-live="polite"><span>{onsite ? job.truckOnSite ? 'On site' : 'Last reported on site' : 'Closest truck now'}</span>{onsite ? <><strong>{onsite}</strong>{job.truckOnSite && onsiteGps && <small>{job.onsiteGpsParked ? "Parked GPS" : "GPS"} {onsiteGps}</small>}{!job.truckOnSite && <small>No departure recorded{job.lastSeenOnsiteAt ? ` · Last GPS ${new Date(job.lastSeenOnsiteAt).toLocaleTimeString('en-US',{timeZone:'America/Chicago',hour:'numeric',minute:'2-digit'})}` : ''}. Awaiting a fresh position.</small>}</> : !isToday ? <strong>Available for today only</strong> : !job.location ? <><strong>{job.addressCheckPending ? 'Locating automatically…' : 'Precise location unavailable'}</strong><small>OpsCenter retries automatically.</small></> : loading ? <strong>Checking distance…</strong> : closest ? <><strong>{truckLabel(closest.truck)} <span>{closest.minutes} min · {closest.miles} mi</span></strong><small>Recent GPS · road estimate. Check availability before assigning.</small></> : <><strong>Unavailable</strong><small>No current GPS and road estimate available.</small></>}</div>}
