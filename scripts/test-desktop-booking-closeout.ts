...
async function main() {
  ...
  try {
    ...
    const closeoutWorker = await fs.readFile(path.join(process.cwd(), 'scripts', 'sync-junkware-job-closeout.ts'), 'utf8');
    assert.match(closeoutWorker, /clickWithWebFormsCompletion\(page, "#ctl00_Content_AdditionalNavigatorsBtn", "the navigator count update"\)/, 'Navigator updates must support JunkWare partial postbacks instead of waiting for a full-page navigation.');
