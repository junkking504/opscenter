...
import type { CustomerSelection } from '../lib/junkware-customer-contract';
...
import type {PrebookingCheck} from '../lib/prebooking-duplicates';
import {appointmentCategory,appointmentStatus} from './lib/schedule-contract';
...
export default function AppointmentCreation({ date, close, saved, onBusyChange }: { date: string; appointments: ScheduleAppointment[]; close: () => void; saved: () => void; onBusyChange: (busy: boolean) => void }) {
  ...
  useEffect(()=>{const body=panel.current?.querySelector('.record-drawer-body');if(body)body.scrollTop=0;},[mode]);
  ...
  const update = <K extends keyof typeof form>(key: K, value: typeof form[K]) => { if (mode !== 'edit'||pending.current) return; setForm(current => ({ ...current, [key]: value, ...(['firstName', 'lastName', 'phone'].includes(key) ? { customerSelection: undefined } : {}) })); setDuplicateCheck(null);setCreateSeparate(false);setError(''); };
  ...
  const review = async () => {
    ...
    const id=crypto.randomUUID();setRequestId(id);setReceipt(null);setError('');setDuplicateCheck(null);setCreateSeparate(false);
    pending.current=true;setBusy(true);
    try {
      const response=await fetch('/api/desktop/schedule/creation/check',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(creationPayload(form,id)),signal:AbortSignal.timeout(20000)});
      const body=await response.json();if(!response.ok||!body.check)throw new Error(body.error||'Duplicate check unavailable. Try reviewing again.');
      setDuplicateCheck(body.check);setMode('review');
  ...
  const submit = async () => {
    if (pending.current || mode !== 'review' || receipt || !duplicateCheck || duplicateCheck.matches.length>0&&(!createSeparate||form.duplicateOverrideReason.trim().length<10)) return;
  ...
  return <><button className="record-drawer-backdrop" aria-label="Close new appointment" disabled={busy} onClick={close} /><aside ref={panel} className="record-drawer appointment-create-drawer" role="dialog" aria-modal="true" aria-labelledby="new-appointment-title" onKeyDown={event => { if (event.key !== 'Tab') return; const elements = [...event.currentTarget.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), select:not(:disabled), textarea:not(:disabled), a[href]')]; const first = elements[0], last = elements.at(-1); if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last?.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first?.focus(); } }}>
    ...
    <fieldset disabled={busy} className="record-drawer-body appointment-create-body">
      {mode === 'edit' && <><CustomerLookup selected={(customer, customerSelection) => { setForm(current => ({ ...current, ...customer, customerSelection, billingSame: customer.billingAddress === customer.serviceAddress && customer.billingZip === customer.serviceZip, howHeard: 'Returning' })); setDuplicateCheck(null); setCreateSeparate(false); setError(''); }} />
      ...
      <section className="appointment-create-section"><header><span>Schedule</span><strong>Place the appointment</strong></header><div className="appointment-create-grid">{select('appointmentType', 'Category', ['Job', 'Estimate'])}{text('date', 'Date', 'date', true)}{select('startTime', 'Start time', Array.from({ length: 10 }, (_, index) => `${index + 8}`.padStart(2, '0') + ':00'))}{select('durationHours', 'Duration (hours)', Array.from({ length: 12 }, (_, index) => String(index + 1)), true)}{select('truck', 'Truck', JUNKWARE_DISPATCH_TRUCKS)}{select('estimatedPickups', 'Pickup-truck loads', Array.from({ length: 12 }, (_, index) => String((index + 1) / 2)), true)}{text('scope', 'Work description', 'text', true)}</div></section>
      ...
      {mode === 'review' && duplicateCheck && <section className="prebooking-check" aria-label="Check Existing Appointments">
        <header><strong>{duplicateCheck.matches.length?'Matching Appointments':'No Matching Appointments Found'}</strong><small>Same customer or phone, same service address, overlapping windows. Checked against the selected day’s snapshot at {new Date(duplicateCheck.observedAt).toLocaleTimeString('en-US',{timeZone:'America/Chicago',hour:'numeric',minute:'2-digit'})}.</small></header>
        {duplicateCheck.matches.map(job=><article key={job.recordId}><div><strong>{job.jkNumber||'JK Unavailable'}</strong><span>{appointmentCategory(job)} · {appointmentStatus(job)}</span></div><strong>{job.customerName}</strong><span> · {job.phone||'Phone Unavailable'}</span><p><a href={'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(serviceAddressForGeocoding(job.address))} target="_self" rel="noopener noreferrer">{job.address}</a></p><p>{job.appointmentTime} · {truckDisplayText(job.truck)} · {job.sourceTerritory||job.territory}</p><a href={'/desktop?workspace=Schedule&scheduleView=board&date='+encodeURIComponent(form.date)+'&appointment='+encodeURIComponent(job.recordId)} target="_self" rel="noopener noreferrer">Open Existing Appointment</a>{job.appointmentUrl&&/^https:\/\/junkware\.junk-king\.com\//i.test(job.appointmentUrl)&&<a href={job.appointmentUrl} target="_self" rel="noopener noreferrer">Review in JunkWare ↗</a>}</article>)}
        {duplicateCheck.matches.length>0&&<><p>These may be intentional. Nothing will be merged, canceled, or reassigned.</p><label className="prebooking-confirm"><input type="checkbox" checked={createSeparate} onChange={event=>setCreateSeparate(event.target.checked)}/> I Reviewed These Bookings and Want to Create a Separate Appointment</label><label><span>Reason for a Separate Appointment</span><Textarea value={form.duplicateOverrideReason} onChange={event=>setForm(current=>({...current,duplicateOverrideReason:event.target.value}))} placeholder="Explain why another appointment is needed (at least 10 characters)" /></label></>}
      </section>}
      {mode === 'review' && <section className="appointment-create-section"><header><span>Review</span><strong>{form.appointmentType} · {form.firstName} {form.lastName}</strong></header><div className="drawer-facts">{[['Date', form.date], ['Window', `${form.startTime} · ${form.durationHours} hours`], ['Truck', form.truck], ['Franchise', form.franchise], ['Address', `${form.serviceAddress}, ${form.serviceZip}`], ['Work', form.scope], ['Phone', form.phone], ['JK Number', 'Assigned by JunkWare after verification']].map(([label, value]) => <div key={label}><span>{truckDisplayText(label)}</span><strong>{truckDisplayText(value)}</strong></div>)}</div><p>Creating this appointment writes to JunkWare. Customer and Krewe messages are separate actions.</p></section>}
      ...
      {busy && <p role="status">{mode === 'edit'?'Checking Existing Appointments…':mode === 'review' ? 'Creating and verifying in JunkWare…' : 'Reading saved result…'}</p>}{error && <p className="appointment-create-error" role="alert">{error}</p>}
    </fieldset><footer className="record-drawer-actions"><Button variant="outline" disabled={busy} onClick={close}>Close</Button>{mode === 'edit' && <Button disabled={busy} onClick={review}>Review Appointment <ArrowRight /></Button>}{mode === 'review' && <><Button variant="outline" disabled={busy} onClick={() => {setMode('edit');setDuplicateCheck(null);setCreateSeparate(false);}}>Edit Details</Button><Button disabled={busy||!duplicateCheck||duplicateCheck.matches.length>0&&(!createSeparate||form.duplicateOverrideReason.trim().length<10)} onClick={submit}>Create in JunkWare</Button></>}{mode === 'result' && receipt?.status === 'failed' && <Button disabled={busy} onClick={() => setMode('edit')}>Review and Correct</Button>}{mode === 'result' && receipt?.status !== 'failed' && receipt?.status !== 'verified' && <Button disabled={busy} onClick={check}>Check Saved Result</Button>}{receipt?.result && <Button onClick={() => { saved(); close(); }}>Return to Schedule</Button>}</footer>
  </aside></>;
}
