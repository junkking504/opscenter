...
import { verifyDesktopAddress, cachedAddressVerification } from '../lib/desktop-address-verification';
...
async function main() {
  ...
  const attempts = read(stateFile).attempts || {};
  ...
  for (const address of candidates) {
    ...
    attempts[hash(address)] = Date.now();
    if (result.location) {
  ...
  write(stateFile, { observedAt: new Date().toISOString(), checked: candidates.length, verified, scheduleDates: dates.size, attempts });
