...
async function main() {
  ...
  try {
    ...
    assert.equal((await mobile.request.post(`${base}/api/truck-inspection`, { data: { action: "connect", truck: "Truck 99", connectionToken: "a".repeat(64) } })).status(), 400);
    await page.goto(`${base}/truck-inspection`);
    await page.getByRole("heading", { name: "Which truck is this?" }).waitFor();
    assert.equal(await page.getByRole("button", { name: "Use this truck" }).isDisabled(), true);
    await page.getByRole("combobox", { name: /This phone’s truck/ }).selectOption("Truck 4");
    ...
    assert.equal(connected.device.truck, "Truck 4");
    assert.deepEqual(connected.inspectors, []);
    assert.equal((await mobile.request.post(`${base}/api/truck-inspection`, { data: { action: "connect", truck: "Truck 8", connectionToken: "b".repeat(64) } })).status(), 409);
    await page.reload();
    await page.getByRole("heading", { name: "Morning, crew." }).waitFor();
    ...
    await stranger.request.post(`${base}/api/truck-inspection`, { data: { action: "connect", truck: "Truck 4", connectionToken: "c".repeat(64) } });
    ...
    console.log("Browser acceptance passed: code-free truck selection, remembered phone, invalid truck and reassignment rejection, no crew directory, Figma mobile layout, double-tap guard, required fuel, operating decision reset on edit, stop receipt, status filters, five checks, problem note/photo, offline draft reload, lost acknowledgement recovery, duplicate safety, report read-back, revoked device and management isolation.");
  } finally {
    await browser.close();
    server.kill("SIGTERM");
