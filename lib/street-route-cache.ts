...
export function loadStreetProgress(directory:string,date:string,truck:string,now=Date.now()):SavedStreetProgress|undefined {
  try {
    ...
    if(!saved.result.paths.every(p=>Number.isInteger(p.sourceEdge) && p.sourceEdge!>=0 && p.sourceEdge!+1<saved.source.points.length && ['matched','estimated','recorded'].includes(p.kind) && Array.isArray(p.points) && p.points.length>=2 && p.points.every(coordinate)))return;
    return saved;
  } catch {return;}
}
