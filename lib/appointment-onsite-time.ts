...
export type AppointmentOnsiteTime = { minutes: number | null; arrival: string | null; departure: string | null; label: string; truck?: string; intervals?: Array<{ arrival: string; departure: string }> };
const truckKey = (value: unknown) => String(value || '').match(/\d+/)?.[0]?.replace(/^0+/, '') || '';
export function appointmentOnsiteTime(job: { appointmentId?: string; jkNumber?: string; truck?: string }, visits: VisitRecord[], now = Date.now(), allowTruckMismatch = false): AppointmentOnsiteTime {
  ...
  const confirmed = byReference.filter(row => row.match_confidence === 'confirmed' && !row.pass_by_only);
  const assignedTruck = truckKey(job.truck);
  const matchingAssignment = confirmed.filter(row => !assignedTruck || truckKey(row.truck_number || row.truck) === assignedTruck);
  const candidates = matchingAssignment.length || !allowTruckMismatch ? matchingAssignment : confirmed;
  const trucks = new Set(candidates.map(row=>truckKey(row.truck_number || row.truck)).filter(Boolean));
  if (!candidates.length || trucks.size > 1) return {...unavailable,label:trucks.size > 1 ? 'Unavailable · GPS visits from multiple trucks' : unavailable.label};
  const matched = candidates;
  const visitTruck = String(matched[0].truck_number || matched[0].truck || '').trim();
  ...
  if (pending) return { ...unavailable, truck: visitTruck || undefined, arrival: !invalid && arrivals.length ? new Date(Math.min(...arrivals)).toISOString() : null, label: 'Awaiting recorded departure' };
  if (invalid || !intervals.length) return { ...unavailable, truck: visitTruck || undefined, label: 'Unavailable · incomplete visit timestamps' };
  ...
  return { minutes, truck: visitTruck || undefined, intervals: merged.map(([start,end]) => ({ arrival: new Date(start).toISOString(), departure: new Date(end).toISOString() })), arrival: new Date(merged[0][0]).toISOString(), departure: new Date(merged.at(-1)![1]).toISOString(), label: `${minutes} min${merged.length > 1 ? ` · ${merged.length} visits` : ''}` };
}
