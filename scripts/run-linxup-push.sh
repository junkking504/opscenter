...
if [[ "${SLACK_OPSCENTER_ALERTS_ENABLED:-false}" =~ ^(1|true|yes|on)$ ]]; then
  cd "$OPSCENTER_DIR"
  node --import tsx scripts/publish-slack-alerts.ts --date "$target_date" --only truck_arrival,linxup_event
fi
