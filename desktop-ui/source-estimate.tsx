...
export function SourceEstimateSummary({ job, showPhotos = false }: { job: ScheduleAppointment; showPhotos?: boolean }) {
  ...
  return <section className="source-estimate" aria-label="Prior estimate">
    ...
    <a href={`https://junkware.junk-king.com/franchise/appointment.aspx?id=${job.sourceEstimateAppointmentId}`} target="_blank" rel="noopener noreferrer">View estimate {estimate?.jkNumber || job.sourceEstimateAppointmentId} ↗</a>
