...
import { assignmentNeedsVerification, appointmentCategory, appointmentRegion, appointmentStatus, isClosed, timelinePlacement, timelineRange, territoryOrder, truckLabel, type ScheduleAppointment, type ScheduleRouting, type ScheduleSnapshot } from './lib/schedule-contract';
import './live-schedule.css';

type Day = 'today' | 'tomorrow';
type Props = { baseDate: string; day: Day; onDayChange: (day: Day) => void; onCounts: (counts: Record<Day, number>) => void; report: (message: string) => void };
...
export default function LiveSchedule({ baseDate, day, onDayChange, onCounts, report }: Props) {
  ...
  const onOperationBusyChange = useCallback((busy: boolean) => { operationBusyRef.current = busy; setOperationBusy(busy); }, []);
  ...
  useEffect(() => {
    ...
    void load();
    const interval = window.setInterval(() => { setNow(new Date()); void load(); }, 60_000);
    return () => { abort.abort(); window.clearInterval(interval); };
  }, [baseDate, refreshKey]);
  useEffect(() => { setSelectedId(null); setDrawerId(null); setPendingMove(null); setScope('ALL'); setPriority(null); setFilter('all'); setRouting(null); }, [date, setDrawerId]);
  ...
  const match = (job: ScheduleAppointment) => {
    ...
    return scopeMatch && (filter === 'all' || filter === 'completed' && appointmentStatus(job) === 'Completed' || filter === 'estimates' && appointmentStatus(job) === 'Estimate Closed' || filter === 'open' && !isClosed(job) || filter === 'unassigned' && truckLabel(job.truck) === 'Unassigned' || filter === 'verify' && !job.location);
  };
  const visible = jobs.filter(match);
  const filtered = filter !== 'all' || scope !== 'ALL';
  const reset = () => { setFilter('all'); setScope('ALL'); setPriority(null); setSelectedId(null); };
  ...
  const notConnected = () => report('Schedule writes are still being connected and verified. No appointment was changed.');
  ...
  return <section className="schedule-workspace live-schedule">
    <div className="schedule-control-bar">
      ...
      <div className="schedule-control-actions"><Button variant="outline" size="sm" onClick={() => setShowMap(value => !value)}>{showMap ? 'Hide Map' : 'Show Map'}</Button><Button size="sm" onClick={notConnected}>New Appointment</Button></div>
    </div>
