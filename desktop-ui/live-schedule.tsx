...
export default function LiveSchedule({ baseDate, day, onDayChange, onCounts, report, view = 'board', onOpenDate, onBusyChange, mapOnly = false }: Props) {
  ...
  const match = (job: ScheduleAppointment) => {
    ...
    return scopeMatch && scheduleMatchesQuery(job, searchQuery) && (filter === 'all' || filter === 'completed' && appointmentStatus(job) === 'Completed' || filter === 'estimates' && appointmentStatus(job) === 'Estimate Closed' || filter === 'open' && !isClosed(job) || filter === 'unassigned' && scheduleDisplayTruck(job) === 'Unassigned' || filter === 'verify' && needsScheduleAddressVerification(job));
  };
  ...
  const addAppointmentAction = <Button className="schedule-add-appointment" variant="brand" size="action" disabled={operationBusy} onClick={() => setCreationOpen(true)}><Plus aria-hidden="true" />Add Appointment</Button>;
