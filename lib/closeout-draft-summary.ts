...
export function closeoutGpsTimes(time: AppointmentOnsiteTime | undefined, visitTruck:string, selectedTruck:string, date:string, fields:Times): Partial<Record<CloseoutTimeKey,string>> {
  ...
  for (const [stamp,hourKey,minuteKey] of [[time.arrival,'actualStartHour','actualStartMinute'],[time.departure,'actualEndHour','actualEndMinute']] as const) {
    ...
    Object.assign(result,closeoutClockFields(stamp,hourKey,minuteKey,date,fields));
  }
  return result;
}

/** Map a recorded clock time to the source system's available time options. */
export function closeoutClockFields(stamp:string,hourKey:'actualStartHour'|'actualEndHour',minuteKey:'actualStartMinute'|'actualEndMinute',date:string,fields:Times):Partial<Record<CloseoutTimeKey,string>> {
  const result:Partial<Record<CloseoutTimeKey,string>>={};
  if(!Number.isFinite(Date.parse(stamp)) || Date.parse(stamp)>Date.now())return result;
  ...
  if (`${parts.year}-${parts.month}-${parts.day}`!==date) return result;
