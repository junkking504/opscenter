...
try {
  ...
  const expected = ['Arrival','New Appointment','Clock In','Duration','Completed'];
  ...
  assert.match(await closed.locator('.crew-event-header').innerText(),/^Completed[\s\S]*New Orleans[\s\S]*JK1000001[\s\S]*8:00 AM – 9:00 AM$/);
  assert.equal(await closed.evaluate(element => getComputedStyle(element).backgroundColor),'rgba(34, 197, 94, 0.5)');
  ...
  assert.deepEqual(await labels(),['Duration','Completed']);
  await page.getByRole('button',{name:'Clear filters',exact:true}).click();
  ...
  assert.deepEqual(await labels(),['Arrival','New Appointment','Truck Unloaded','Clock In','Duration','Completed','Inspection'],'Source updates missing from appointment indexes are retained; timezone offsets sort by instant and unknown times sort last');
  await page.getByRole('button',{name:'Reset preview'}).click();
  await page.getByRole('button',{name:'Simulate warehouse entry'}).click();
  assert.deepEqual(await labels(),['Arrival','New Appointment','Geofence Entry','Clock In','Duration','Completed'],'Native geofence entries interleave by occurrence time');
