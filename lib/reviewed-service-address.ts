...
export function reviewedServiceAddress(address: string): { location: PlanningLocation; reason: string; verifiedAddress: string } | undefined {
  ...
  const premises = reviewedAddressIdentity(withoutServiceUnit(address));
  ...
  for (const key of new Set([identity,premises])) try {
    const row = JSON.parse(fs.readFileSync(path.join(root, 'cache/service-address-reviews', createHash('sha256').update(key).digest('hex') + '.json'), 'utf8'));
    const point = row.location;
    // Existing unit-specific records never silently become property-wide.
    // Only an explicitly reviewed premises record may serve other units.
    if (key !== identity && row.scope !== 'premises') return undefined;
    if (row.schema !== 1 || row.status !== 'verified' || reviewedAddressIdentity(row.originalAddress) !== key || !row.verifiedAddress || !Array.isArray(row.sources) || !row.sources.length || !row.sources.every((url: unknown) => typeof url === 'string' && /^https:\/\//.test(url))) return undefined;
