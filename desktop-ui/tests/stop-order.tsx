...
function App(){const [snapshot,setSnapshot]=React.useState(initial);return <main className="ops-live"><div className="schedule-board-layout map-open"><div className="schedule-board-actions"><ScheduleStopOrder snapshot={snapshot} saved={setSnapshot} busy={false} onBusyChange={()=>{}}/></div></div><output>{[...snapshot.appointments].sort(compareStops).map(job=>job.jkNumber).join(',')}</output></main>;}
