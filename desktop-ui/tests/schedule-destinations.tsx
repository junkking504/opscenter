...
const progressGps=()=>new Date(Date.now()-(progressStage==='stale'?300000:20000)).toISOString();
...
window.fetch=async(input,init)=>{
  ...
  if(url.pathname==='/api/desktop/schedule')return Response.json({date,observedAt:'2026-09-07T21:00:00Z',sourceRequest:{state:'ready',message:''},appointments:appointments(date),fleet:{isToday:progressEnabled || scenario==='on-site' || longDetails && date==='2026-09-07',lastUpdatedAt:null,trucks:progressEnabled?[{truck:'Truck 9',latitude:30.45,longitude:-91.18,lastGpsUpdate:progressGps(),freshnessLabel:'Recent GPS',operationalStatus:'Available',driver:'Example driver',navigator:'',serviceStatus:'Available'}]:scenario==='on-site'?[{truck:'Truck 8',latitude:30.45,longitude:-91.18,lastGpsUpdate:new Date(Date.now()-300_000).toISOString(),freshnessLabel:'GPS Stale',operationalStatus:'GPS Stale',ignition:'OFF',driver:'Example Driver',navigator:'Example Navigator',serviceStatus:'Unavailable'}]:[]}});
