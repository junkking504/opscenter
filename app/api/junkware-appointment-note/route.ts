...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "The appointment note was not valid." }, { status: 400, headers: { "Cache-Control": "no-store, max-age=0" } });
  }
  if (!/^\d{1,12}$/.test(appointmentId)) return NextResponse.json({ ok: false, error: "The JunkWare appointment is unavailable." }, { status: 400, headers: { "Cache-Control": "no-store, max-age=0" } });

  try {
    const junkware = await withJunkwareAppointmentSyncLock(appointmentId, () => addJunkwareAppointmentNote({ appointmentId, note }));
    return NextResponse.json({ ok: true, junkware }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    ...
    return NextResponse.json({ ok: false, error: detail }, { status: 502, headers: { "Cache-Control": "no-store, max-age=0" } });
  }
}
