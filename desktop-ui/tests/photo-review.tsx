...
window.fetch=async (input,options) => {
  ...
  const rows:PhotoReviewRecord[]=Array.from({length:advanced?27:28},(_,i)=>({
    id:i.toString(16).padStart(4,'0').padEnd(64,'0'),state:i===0?'failed':'review',receivedAt:`2026-09-${String(i%4+1).padStart(2,'0')}T15:00:00Z`,outcomeAt:null,jobDate:`2026-09-${String(i%4+1).padStart(2,'0')}`,sender:i===0?'Sender ending 2222':'Sender ending 1111',senderKey:i===0?'sender-two':'sender-one',jk:i===0?'JK7654321':'JK1234567',caption:i===0?'Synthetic after photo':'Synthetic before photo',category:'Before',reason:i===0?'network_request_failed':'sender_not_mapped_to_truck',reasonLabel:i===0?'Network request failed':'Sender mapping needed',nextStep:'Verify the original message and intended appointment before recovery.',attempts:0,previewAvailable:false,sourceHref:null
  }));
