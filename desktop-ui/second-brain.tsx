...
export default function SecondBrain({ workspace, disabled, navigate }: { workspace: string; disabled: boolean; navigate: (workspace: string) => void }) {
  ...
  return <>
    <button className="brain-launch" disabled={disabled} onClick={() => { setOpen(true); link(selectedId || '1'); }}><BookOpen size={17} /><span>Second Brain</span></button>
    {open && <dialog ref={dialog} className="brain-dialog" aria-labelledby="brain-title" onKeyDown={event => event.stopPropagation()} onCancel={event => { event.preventDefault(); close(); }}>
      <header className="brain-header"><div><p className="brain-eyebrow">OPSCENTER KNOWLEDGE</p><h2 id="brain-title">Second Brain</h2><p>Past discussions, recurring issues, fixes, and decisions.</p></div><button className="brain-icon" aria-label="Close Second Brain" disabled={locked} onClick={() => close()}><X size={22} /></button></header>
      <div className="brain-body" ref={content}>
        ...
        {draft ? <form className="brain-editor" onSubmit={event => { event.preventDefault(); void submit({ action: 'save', id: editId!, expectedVersion: editVersion, requestId: crypto.randomUUID(), draft }); }}>
          ...
          {!snapshot ? <p role="status">{error ? 'Knowledge could not be loaded.' : 'Loading knowledge…'}</p> : <><p className="brain-count" role="status">{rows.length} {rows.length === 1 ? 'entry' : 'entries'}{scope !== 'All' ? ` relevant to ${workspaceLabel(scope)}` : ' across OpsCenter'} · Search includes steps, source references, and owners.</p>
            <div className="brain-results">{rows.slice(0, pageSize).map(item => <button className="brain-card" key={item.id} onClick={() => select(item.id)}><div className="brain-tags"><span>{kindLabel[item.kind]}</span><span>{workspaceLabel(item.workspace)}</span><span className={`brain-status brain-status-${item.status}`}>{knowledgeStatus(item)}</span></div><h3>{item.title}</h3>{item.learning && <span className="brain-outcome">{outcomeLabel[item.learning.outcome]} · {date(item.learning.recordedAt)}</span>}<p>{item.summary}</p><small>{item.sourceLabel} · {date(item.updatedAt)}</small></button>)}</div>
            {rows.length > pageSize && <button onClick={() => setPageSize(value => value + 40)}>Show more records ({rows.length - pageSize} remaining)</button>}
            {!rows.length && <div className="brain-empty"><h3>No entries match these filters.</h3><p>Try another phrase or look across all workspaces.</p><button onClick={() => { setQuery(''); setScope('All'); setKind('all'); setStatus('all'); setOutcome('all'); setPageSize(40); }}>Reset filters</button></div>}
