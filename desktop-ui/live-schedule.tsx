...
import { ArrowRight, GripVertical, LockKeyhole, X } from 'lucide-react';
  ...
  return <TruckCameraController className="live-schedule-camera"><section className={`schedule-workspace live-schedule${mapOnly ? ' command-map-content' : ' schedule-dispatch'}`}>
    ...
    <div className={`schedule-board-layout${showMap ? ' map-open' : ''}`}>
      ...
      {!mapOnly && <div className="schedule-board-shell"><div className="section-title"><div><span className="section-kicker">{day === 'today' ? 'Today' : 'Tomorrow'} · JunkWare Snapshot</span><h2>Truck Schedule</h2></div><div className="schedule-board-actions"><span className="schedule-drag-help"><GripVertical size={13} />Drag Appointment → Truck + Time</span></div></div>
        <div className="schedule-board-scroll"><div className={`schedule-board ${truckNames.length >= 10 ? 'ultra' : truckNames.length >= 7 ? 'compact' : 'comfortable'}`} style={{ minWidth: 144 + ticks.length * 120, '--schedule-hour-count': ticks.length } as CSSProperties}>
          <div className="schedule-time-row" style={{ gridTemplateColumns: `144px repeat(${ticks.length}, minmax(0, 1fr))` }}><span>Route</span>{ticks.map(tick => <span key={tick}>{clock(tick)}</span>)}</div>
          {date === today && progress >= 0 && progress <= 1 && <div className="schedule-now-line" style={{ left: `calc(${progress * 100}% + ${(1 - progress) * 144}px)` }} aria-label={`Current time ${clock(nowMinutes)}`}><span>{clock(nowMinutes)}</span></div>}
          {truckNames.map((truck, index) => {
            ...
            return <div className="schedule-truck-row" data-schedule-truck={truck} key={truck} style={{ height: placed.length ? laneCount * 96 + 20 : 60 }}><button type="button" className="schedule-truck-cell" aria-label={`Select ${truck} on map`} aria-pressed={selectedTruck === truck} onClick={() => selectTruck(truck)}><i className={['blue', 'red', 'gold', 'purple'][index % 4]} /><strong>{truck}</strong><span>{rowJobs[0] ? crew(rowJobs[0]) : 'No Scheduled Work'}</span></button><div className="live-truck-timeline">
              {placed.map(({ job, position, lane }) => <div key={job.recordId} className={`schedule-appointment territory-${appointmentRegion(job).code.toLowerCase()} ${slug(appointmentCategory(job))} ${slug(appointmentStatus(job))}${filtered ? match(job) ? ' scope-match' : ' scope-muted' : ''}${selectedId === job.recordId ? ' route-selected' : ''}${drag.preview?.job.recordId === job.recordId ? ' is-dragging' : ''}`} style={{ left: `${position.left * 100}%`, width: `calc(${position.width * 100}% - 2px)`, top: lane * 96 + 4, height: 88 }} role="group" tabIndex={0} aria-roledescription={isClosed(job) || assignmentNeedsVerification(job) || operationBusy ? undefined : "draggable appointment"} onPointerDown={event => drag.begin(event, job)} aria-label={`${job.jkNumber} · ${job.customerName} · ${job.appointmentTime}`} aria-description={scheduleMoveRestriction(job) || "Drag to change truck or time; click for appointment details."} onClick={() => { if (!drag.suppressClick.current) selectAppointment(job.recordId); }} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectAppointment(job.recordId); } }} title={`${job.jkNumber} · ${job.customerName} · ${job.appointmentTime} · ${appointmentRegion(job).label} · ${appointmentStatus(job)}${scheduleMoveRestriction(job) ? ` · ${scheduleMoveRestriction(job)}` : ""}`}><span>{job.appointmentStartMinutes != null && job.appointmentEndMinutes != null ? `${clock(job.appointmentStartMinutes)}–${clock(job.appointmentEndMinutes)}` : job.appointmentTime}</span>{!isClosed(job) && !assignmentNeedsVerification(job) && <GripVertical className="schedule-grip" size={10} aria-hidden="true" />}<button type="button" className="jk-record-link schedule-jk-link" onClick={event => { event.stopPropagation(); if (!drag.suppressClick.current) selectAppointment(job.recordId); }}>{/^JK/.test(job.jkNumber) ? <>JK<wbr />{job.jkNumber.slice(2)}</> : job.jkNumber}</button><small>{job.customerName}</small><em className="schedule-block-status">{scheduleMoveRestriction(job) && <LockKeyhole size={10} aria-hidden="true" />}{assignmentNeedsVerification(job) ? "Verify Assignment" : appointmentStatus(job)}</em></div>)}
              {displayLegs.filter(leg => truckLabel(leg.truck) === truck && leg.gapMinutes > 0).map(leg => { const to = jobs.find(job => job.recordId === leg.toAppointmentId)!; const end = to?.appointmentStartMinutes; if (end == null) return null; return <div key={`${leg.fromAppointmentId}:${leg.toAppointmentId}`} className={`schedule-route-leg ${leg.bufferMinutes !== null && leg.bufferMinutes < 0 ? 'late' : leg.travelMinutes === null ? 'unavailable' : 'clear'}`} style={{ left: `${(end - leg.gapMinutes - range.start) / range.duration * 100}%`, width: `${leg.gapMinutes / range.duration * 100}%` }} title={`${leg.fromJk} → ${leg.toJk}: ${leg.travelMinutes === null ? unavailableRoute(leg, jobs).detail : `${leg.travelMinutes} minutes · ${leg.miles} miles · ${leg.bufferMinutes} minute window gap after travel`}`}><strong>{leg.travelMinutes === null ? unavailableRoute(leg, jobs).label : `${leg.travelMinutes}m · ${leg.miles}mi`}</strong></div>; })}
