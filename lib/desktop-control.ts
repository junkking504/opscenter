...
export function controlReconciliationReady(date: string, metrics: Record<string, unknown> | null, now = new Date()): boolean {
  ...
  if (date > chicagoDate(now)) return false;
...
export async function readDesktopControl(dateInput: string, identity: Identity, page = 1, focusItemId?: string): Promise<ControlSnapshot> {
  ...
  const [itemRows, allActiveRows, command, day, countResult, audits, actions, actionRows] = await Promise.all([
    ...
    getKernelPool().query<{ count: string }>(`SELECT count(*) FROM opscenter_kernel.work_items WHERE (operating_date=$1 OR (operating_date<$1 AND (status NOT IN ('resolved','dismissed') OR (COALESCE(resolved_at,updated_at) AT TIME ZONE 'America/Chicago')::date=$1))) AND ($2 OR category <> 'Finance')`, [date,canFinance]),
  ...
  const items = itemRows.rows.map(({item,rule}) => ({ ...item, rule, currentSource:currentEvidence(item), ownerDisplayName: item.ownerActorId ? owners.get(item.ownerActorId) : undefined, recommendedAction: inboxRulePolicy(rule).recommendedAction, href: desktopWorkItemHref(item) }));
