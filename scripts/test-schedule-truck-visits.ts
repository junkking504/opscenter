...
assert.deepEqual(scheduleBoardJobs([sharedPremises],'Truck 3',now).map(row=>row.appointmentId),['1'],'Truck 3 gets its own measured visit block');
assert.deepEqual(scheduleBoardJobs([sharedPremises],'Truck 8',now).map(row=>row.appointmentId),['1'],'Truck 8 retains its own measured visit history');
assert.equal(scheduleTravelLayout(scheduleBoardJobs([sharedPremises],'Truck 3',now),[],range,'Truck 3',now).placed.length,1,'Board layout projects confirmed visits into the physical truck lane');
...
assert.equal(scheduleBoardJobs([completedMismatch],'Truck 1',now).length,0,'A completed mismatch does not create a planned block on the stale JunkWare lane');
