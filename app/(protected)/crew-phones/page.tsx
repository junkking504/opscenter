...
export default function CompanyPhones() {
  ...
  return <main className={styles.page}><div className={styles.content}>
    <h1>Company phones</h1><p>Crew app: <a href={CREW_JOBS_ORIGIN}>jobs.junk-king.app</a></p><p>Set up job access on company-issued phones only. Each connection belongs to one truck.</p>
    ...
    {enrollment && <section className={styles.card} aria-live="polite"><h2>{enrollment.label}</h2><p>{enrollment.truck}</p><p>On the company phone, open <a href={CREW_JOBS_ORIGIN}>jobs.junk-king.app</a> and enter this 6-digit code:</p><p className={styles.code}>{enrollment.code}</p><p>Expires {new Date(enrollment.expiresAt).toLocaleTimeString([], {hour:'numeric', minute:'2-digit'})}. Usable by one phone.</p><button className={styles.secondary} disabled={busy} onClick={() => void send({ action: 'revoke', deviceId: enrollment.deviceId })}>Cancel this setup code</button></section>}
