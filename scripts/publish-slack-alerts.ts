...
function selectedKinds(): SlackAlertKind[] | undefined {
  ...
  if (!kinds.length || !kinds.every((kind) => ["truck_inspection", "truck_arrival", "truck_departure", "geofence_entry", "geofence_exit", "job_closed", "estimate_closed"].includes(kind))) {
    throw new Error("--only supports truck_inspection alone, truck_arrival/truck_departure, geofence_entry/geofence_exit, or job_closed/estimate_closed.");
  }
