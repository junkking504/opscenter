...
function Fixture() { const [scope, setScope] = useState<AnalyticsScope>('expenses'); return <div className="ops-live"><main className="finance-performance" style={{ maxWidth: 1300, margin: '20px auto' }}><h2>Synthetic operating chart fixture</h2><nav aria-label="Chart page fixture">{Object.keys(scopeMetrics).map(key => <button key={key} onClick={() => setScope(key as AnalyticsScope)} style={{ padding: 12 }}>{key}</button>)}</nav><OperatingTrends key={scope} data={data} scope={scope} /></main></div>; }
