import type { DrivingScore } from './driving-score';
...
export type FleetView = 'scores' | 'overview' | 'maintenance' | 'service' | 'reports';
...
export type DesktopCrewMember = CrewAmounts & { drivingScores?: DrivingScore[]; id: string; name: string; initials: string; role: string; truck: string; working: boolean; clockIn: string; clockOut: string; clockInAt?: number | null; clockOutAt?: number | null; hourlyRate: number | null; status: string; issue: string; payNote?: string; junkwareSync?: {status:'pending'|'verified'|'failed'|'uncertain';message:string;verifiedAt?:string}|null; version: string; actionVersions?: { correction: string; bonus: string }; correction: { clockIn: string; clockOut: string; hourlyRate: number; note: string; updatedBy: string; updatedAt: string } | null; days: Array<CrewAmounts & CrewDayEvidence & { date: string; clockIn: string; clockOut: string }> };
export type DesktopKreweDay = { date: string; canWrite: boolean; member: DesktopCrewMember; manualBonuses: Array<{entryId: string; amount: number; note: string}> };
...
export type DesktopKreweSnapshot = { hoursSnapshot?: KreweHoursSnapshot; excludedPayNames?: string[]; date: string; view: KreweView; start: string; end: string; sourceUpdatedAt: string | null; missingDates: string[]; payrollVisible: boolean; canWrite: boolean; members: DesktopCrewMember[]; totals: CrewAmounts; callIn: null | { targetDate: string; scheduleAvailable: boolean; appointmentCount: number; requiredCrews: number; requiredHeadcount: number; alreadyAssignedHeadcount: number; callInCount: number; assumedShiftHours: number; note: string; territoryDemand: Array<{ territory: string; appointments: number; crews: number }>; recommendations: Array<{ name: string; rank: number; suggestedRole: string; weeklyHours: number; projectedWeeklyHours: number; recentRph: number; recentJobs: number; overtimeRisk: boolean; reason: string; decision: CallInDecision | null; version: string }> } };
...
export type DesktopFleetTruck = { id: string; label: string; vehicle: string; readiness: string; operatingStatus: string; driver: string; navigator: string; assignment: string; location: string; gpsAt: string | null; speed?: number | null; ignition?: string | null; gpsFreshness: string; odometer: string; serviceStatus: string; nextService: string; checklist: string; inspectionSource?: string; inspectionHref?: string; inspectionFindings?: Array<{ label: string; notes: string }>; loadPercent: number | null; loadLabel?: string; loadNeedsVerification?: boolean; loadNote: string; loadVersion: string; checklistVersion: string; checklists: Record<'daily' | 'weekly' | 'monthly', DesktopChecklist>; checklistDefinitions: Array<{ itemId: string; label: string; guidance: string }>; answers: Array<{ itemId: string; status: string; notes: string }>; jobs: number | null; revenue: number | null; miles: number | null; idleMinutes: number | null; driverScore: number | null };
export type DesktopFleetSnapshot = { drivingScores?: DrivingScore[]; date: string; report: string; sourceUpdatedAt: string | null; sourceAvailable: boolean; canWrite: boolean; trucks: DesktopFleetTruck[]; issues: FleetIssueRow[]; maintenance: FleetMaintenanceRow[]; reportRows: Array<{ truck: string; jobsCompleted: number; revenue: number; miles: number; idleTimeMinutes: number; averageDriverScore: number | null }>; reportCoverageDays: number; warnings: string[] };
