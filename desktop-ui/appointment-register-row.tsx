...
export function AppointmentRegisterRow({ job, selected, area, warning, route, proximity, select, open }: {
  ...
  return <article className={`appointment-register-row readable-appointment ${status.toLowerCase().replaceAll(' ', '-')}${selected ? ' route-selected' : ''}`} role="row" aria-selected={selected} onClick={event => { if (!(event.target as HTMLElement).closest('a,button')) select(); }}>
    ...
    <div className={`register-payment payment-${payment.tone}`} role="cell" aria-label={`Payment for ${job.jkNumber}`}><span className="register-payment-label">{payment.label}</span>{payment.amount && <strong className="register-payment-amount">{payment.amount}</strong>}{payment.details.map(detail => <span className="register-payment-detail" key={detail}>{detail}</span>)}{payment.balance && <span className="register-payment-balance">{payment.balance}</span>}</div>
