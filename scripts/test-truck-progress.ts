...
assert.equal((await get([job('onsite',{truckOnSite:true}),second],[parked]))[0].status,'on_site','A matched on-site appointment retains the valid parked reporting window');
assert.equal(currentOnsiteTruckGps(parked,now),true,'The browser and progress API share the same on-site freshness rule');
