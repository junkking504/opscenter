...
export function combinedCloseoutAlerts(messages: SlackDigestMessage[], completed: AnyRecord[], reconciliation: PaymentReconciliationView, crewCorrections?: Parameters<typeof applyClockOutCorrections>[1]): OperationalAlert[] {
  ...
  const isJobCloseout = (alert: OperationalAlert) => ['Job Closed', 'Job Completed'].includes(alert.label);
  const combinesWithCloseout = (alert: OperationalAlert) => isJobCloseout(alert) || alert.label === 'Payment Recorded';
  const closeouts = new Map<string, OperationalAlert>();
  for (const alert of presented) {
    const key = jobKey(alert);
    if (key && isJobCloseout(alert) && !closeouts.has(key)) closeouts.set(key, alert);
  }
  return presented.flatMap(alert => {
    const key = jobKey(alert);
    const canonical = key ? closeouts.get(key) : undefined;
    if (canonical && alert !== canonical && combinesWithCloseout(alert)) return [];
    if (!isJobCloseout(alert)) return [alert];
    ...
    return [{ ...alert, sourceMessageIds: Array.from(new Set(presented.filter(item => jobKey(item) === key && combinesWithCloseout(item)).flatMap(item => item.sourceMessageIds || [item.id]))), facts: [...facts, ...paymentFacts], next: 'Review closeout, payment verification, and photos.' }];
  });
}
