...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!(await dateField.count()) || !(await startField.count()) || !(await availableTimes.count()) || !(await save.count())) {
      throw new Error("The JunkWare reschedule controls have changed.");
    }
    ...
    if (previousDate !== expectedDate || previousAppointmentStartMinutes !== expectedAppointmentStartMinutes) {
      throw new Error("VERSION_CONFLICT: The JunkWare appointment date or time changed before this request was approved.");
    }
    ...
    if (changed) {
      ...
      if (!(await availableTimes.locator(`option[value="${timeValue}"]`).count())) {
        throw new Error("That start time is not available for this appointment in JunkWare.");
      }
      ...
      if (selectedDate !== date || selectedStart !== appointmentStartMinutes) {
        throw new Error("JunkWare did not retain the requested date and time before the update.");
      }
    ...
    if (verifiedDate !== date || verifiedAppointmentStartMinutes !== appointmentStartMinutes) {
      throw new Error("JunkWare did not retain the requested appointment date and time.");
    }
