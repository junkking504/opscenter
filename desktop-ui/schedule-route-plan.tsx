...
export default function ScheduleRoutePlan({snapshot,busy,select,review}: {snapshot:ScheduleSnapshot;busy:boolean;select:(id:string)=>void;review:(job:ScheduleAppointment,truck:string)=>void}) {
  ...
  return <section className="schedule-route-plan" aria-label="Route Planner">
    ...
    {open&&<>
      ...
      {plan&&<>
        <div className="route-plan-explainer"><strong>Proposal Only · Nothing Applied</strong><span>OpenStreetMap road estimates · No live traffic · Calculated {new Date(plan.calculatedAt).toLocaleTimeString('en-US',{timeZone:'America/Chicago',hour:'numeric',minute:'2-digit'})}. Planned arrivals assume the entered service time and start at the first stop—not at the truck’s GPS position. Missing travel makes downstream arrivals unknown.</span><span>{plan.excluded} closed, unverified, unidentified, or location-review appointments excluded. Stop order is not saved to JunkWare; Move Appointment changes only the truck and verifies JunkWare in the background. Appointment windows stay unchanged.{historical?' Historical date: assignments cannot be applied from this proposal.':''}</span></div>
        <div className="route-plan-routes">{(draft||plan.routes).map(route=>{
          ...
          return <article className="route-plan-truck" key={route.truck}><header><h3>{truckDisplayText(route.truck)}</h3><span>{route.appointmentIds.length} Stops{valid&&route.appointmentIds.length>1?` · ${allKnown?'':'Partial: '}${legs.reduce((sum,s)=>sum+(s.travelMinutes||0),0)} min · ${legs.reduce((sum,s)=>sum+(s.miles||0),0).toFixed(1)} mi between stops`:''}{valid&&warnings?` · ${warnings} Need Review`:''}</span></header>
            ...
            <ol>{route.appointmentIds.map((id,index)=>{
              ...
              return <li key={id}>
                {index>0&&<div className="route-plan-leg">↓ {valid&&stop?(stop.travelMinutes===null?'Travel estimate unavailable':`${stop.travelMinutes} min · ${stop.miles} mi from previous stop`):'Recalculate travel'}</div>}
                <div className="route-plan-stop"><span className={`route-plan-number territory-${region.code.toLowerCase()}`}>{index+1}</span>
                  <div className="route-plan-detail"><button className="route-plan-reference" disabled={busy} onClick={()=>select(id)}>{job.jkNumber||'JK Pending'} · {job.customerName||'Customer Unavailable'}</button><span>{job.appointmentTime||'Time Not Set'} · {region.label} · {job.appointmentType}</span><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(appointmentServiceAddress(job))}`} target="_self" rel="noopener noreferrer">{appointmentServiceAddress(job)||'Address Unavailable'}</a><small>{valid&&stop?`${index===0?'Planned Start':'Planned Arrival'}: ${time(stop.arrival)}${stop.warnings.length?` · ${stop.warnings.join(' · ')}`:''}`:'Estimates Need Recalculation'}</small></div>
                  <div className="route-plan-stop-actions"><div><Button size="sm" variant="outline" aria-label={truckDisplayText(`Move ${job.jkNumber} earlier in ${route.truck}`)} disabled={loading||busy||stale||!draft||index===0} onClick={()=>adjust(id,route.truck,-1)}><ArrowUp size={14}/></Button><Button size="sm" variant="outline" aria-label={truckDisplayText(`Move ${job.jkNumber} later in ${route.truck}`)} disabled={loading||busy||stale||!draft||index===route.appointmentIds.length-1} onClick={()=>adjust(id,route.truck,1)}><ArrowDown size={14}/></Button></div><select aria-label={truckDisplayText(`Proposed truck for ${job.jkNumber}`)} value={route.truck} disabled={loading||busy||stale||!draft} onChange={e=>adjust(id,e.target.value)}>{trucks.map(t=><option key={t} value={t}>{truckDisplayText(t)}</option>)}</select>{changed?<Button variant="outline" size="sm" disabled={!valid||historical||busy} onClick={()=>review(job,route.truck)}>Move Appointment</Button>:<small>Current Assignment</small>}</div>
                </div>
              </li>;
            })}</ol>
          </article>;
