...
export type VisitEvidence = 'native_geofence' | 'live_position' | 'later_facility' | 'confirmed_appointment' | 'operational_confirmation';
export type TrackedVisit = {
  ...
  appointmentId?: string; jobNumber?: string; conflict?: boolean;
};
...
export function trackGeofenceVisits(date: string, transitions: GeofenceTransition[], now = Date.now()): TrackedVisit[] {
  const events = [...new Map(transitions.filter(event => Number.isFinite(Date.parse(event.timestamp)) && Date.parse(event.timestamp) <= now)
    ...
    .sort((a,b) => a.timestamp.localeCompare(b.timestamp) || Number(a.type === 'position') - Number(b.type === 'position') || a.type.localeCompare(b.type));
  ...
  for (const event of events) {
    ...
    if (event.type === 'exit') {
      ...
      const inferred = !visit && [...completed].reverse().find(candidate=>candidate.truck === event.truck && facilityKey(candidate.name) === facilityKey(event.name) && candidate.departureSource === 'later_facility');
    ...
    if (previous) {
      ...
      active.set(key,{...previous,enteredAt:ambiguous ? null : previous.enteredAt,
      ...
      active.set(key,{agentId:VISIT_TRACKING_AGENT,id:event.id,kind:'geofence',truck:event.truck,name:event.name,
        facility:event.facility,resetLocation:event.resetLocation,enteredAt:event.timestamp,departedAt:null,lastSeenAt:event.timestamp,
...
export type AppointmentVisitSource = {
  ...
  first_arrival?: string; final_departure?: string; visit_count?: number;
  visit_intervals?: {arrival?: string; departure?: string | null; departure_confirmed?: boolean}[];
  ...
  for (const visit of visits) {
    ...
    for (const interval of intervals) {
      ...
      candidates.set(key,{agentId:VISIT_TRACKING_AGENT,id:`appointment-visit-${createHash('sha256').update(key).digest('hex').slice(0,24)}`,
        ...
        firstObservedAt:start,enteredAt:start,departedAt:conflict ? null : departedAt,lastSeenAt:departedAt || start,entryIds:[],
