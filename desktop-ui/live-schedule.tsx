...
export default function LiveSchedule({ baseDate, day, onDayChange, onCounts, report, view = 'board', onOpenDate, onBusyChange, mapOnly = false }: Props) {
  ...
  const closestTruckFor = (job: ScheduleAppointment) => {
    ...
    return routing.closest.find(candidate => candidate.status === 'available' && candidate.miles !== null && candidate.minutes !== null) || null;
  };
      ...
      {showMap && <section ref={mapPanelRef} className="schedule-map-panel">
        ...
        <aside className="schedule-map-controls"><label className="schedule-gps-picker">Truck GPS route<select aria-label="Truck GPS route" value={selectedTruck || ''} onChange={event=>event.target.value?selectTruck(event.target.value):setSelectedTruck(null)}><option value="">Select truck</option>{truckNames.map(truck=><option key={truck} value={truck}>{truck}</option>)}</select></label>{!selectedTruck && !selected && <><div><span className="section-kicker">{snapshot.fleet.isToday ? 'Live Map' : 'Planning Map'}</span><h2>{snapshot.fleet.isToday ? 'Dispatch Positions' : 'Appointment Coverage'}</h2></div>
          ...
          {selectedTruck ? <section className="live-map-truck-details" aria-label={`${selectedTruck} details`}>
            ...
            <div className="map-selected-appointment" aria-live="polite"><strong>{selected.customerName}</strong><PartnerBadge job={selected} /><span>{selected.appointmentTime} · {truckLabel(selected.truck)}{selected.truckOnSite && selected.onsiteTruck && truckLabel(selected.onsiteTruck) !== truckLabel(selected.truck) ? ` · ${selected.onsiteTruck} on site` : ''}</span><Address value={selected.address} /><span>{appointmentStatus(selected)} · {selected.junkItems.join(' · ') || appointmentCategory(selected)}</span><strong className="selected-appointment-proximity">{closestTruckText(selected)}</strong>{!selected.location && <p role="status">Verify Address · This appointment has no verified map pin.</p>}</div>
            <div className="section-kicker">Closest Trucks</div>
    ...
    <section className="appointment-register"><div className="section-title appointment-register-title"><div><span className="section-kicker">{visible.length} Shown · {selectedTruck || (scope === 'ALL' ? 'All Territories' : territoryLabels[scope.split(':')[0]] || scope)}</span><h2>All Appointments</h2></div>{filtered && <Button variant="outline" size="sm" onClick={reset}>Show All {jobs.length}</Button>}</div>
      ...
      <div className="appointment-register-table" role="table" aria-label="All scheduled appointments">{[...groups].sort((a, b) => a.code === priority ? -1 : b.code === priority ? 1 : territoryOrder.indexOf(a.code) - territoryOrder.indexOf(b.code)).map(group => {
        ...
        return <section className={`appointment-territory-group ${slug(group.label)}`} role="rowgroup" key={group.code}><div className="appointment-territory-heading"><div><i /><button onClick={() => focusTerritory(group.code)}>{group.code}</button><button className="territory-record-link" onClick={() => focusTerritory(group.code)}>{group.label}<ArrowRight size={11} /></button></div><span>{groupJobs.length} Appointments</span></div><div className="appointment-register-head" role="row"><span>Time</span><span>Appointment</span><span>Customer</span><span>Service Location</span><span>Assignment</span><span>Work</span><span>Status and Notes</span></div>
          {[...new Set(groupJobs.map(region => region.areaCode))].map(areaCode => { const areaJobs = groupJobs.filter(region => region.areaCode === areaCode); return <section className={`appointment-area-group ${areaCode.toLowerCase()}`} key={areaCode}><div className="appointment-area-heading"><div><button onClick={() => focusTerritory(`${group.code}:${areaCode}`)}>{areaCode}</button><button className="area-record-link" onClick={() => focusTerritory(`${group.code}:${areaCode}`)}>{areaJobs[0].area}<ArrowRight size={10} /></button></div><span>{areaJobs.length} Appointments</span></div>{areaJobs.map(({ job, area, mismatch, needsReview, reason, sourceTerritory }) => { const leg = routeTo(job.recordId); const proximity = closestTruckText(job); const onsiteMismatch = job.truckOnSite && job.onsiteTruck && truckLabel(job.onsiteTruck) !== truckLabel(job.truck); return <article className={`appointment-register-row ${slug(appointmentStatus(job))}${selectedId === job.recordId ? ' route-selected' : ''}`} aria-selected={selectedId === job.recordId} role="row" key={job.recordId} onClick={event => { if (!(event.target as HTMLElement).closest('a,button,input,select,textarea')) selectAppointment(job.recordId); }}><div><strong>{job.appointmentTime || 'Time Not Set'}</strong><small>{appointmentCategory(job)}</small></div><div><button className="jk-record-link" onClick={() => selectAppointment(job.recordId)}>{job.jkNumber || 'JK Pending'}</button><PartnerBadge job={job} /></div><div><strong>{job.customerName || 'Customer Unavailable'}</strong><Phone value={job.phone} /></div><div><Address value={job.address} /><small>{area}</small>{(mismatch || needsReview) && <small title={reason}>{needsReview ? reason : `JunkWare Franchise: ${sourceTerritory} · Address-Based Territory`}</small>}{leg && <small className={`route-leg-inline${leg.bufferMinutes !== null && leg.bufferMinutes < 0 ? ' late' : ''}`}>{leg.travelMinutes === null ? unavailableRoute(leg, jobs).label : `${leg.travelMinutes} min · ${leg.miles} mi`} from {leg.fromJk}{leg.bufferMinutes !== null ? ` · ${leg.bufferMinutes < 0 ? `${Math.abs(leg.bufferMinutes)}m short` : `${leg.bufferMinutes}m window gap`}` : ''}</small>}</div><div><strong>{truckLabel(job.truck)}</strong><small>{crew(job)}</small>{onsiteMismatch && <small className="assignment-unverified">{job.onsiteTruck} on site</small>}{assignmentNeedsVerification(job) && <small className="assignment-unverified">Assignment Not Verified</small>}</div><div><strong>{job.junkItems.join(' · ') || appointmentCategory(job)}</strong><small>{appointmentCategory(job) === 'Estimate' ? `${money(job.paymentAmount)} quoted` : job.paymentAmount ? `${money(job.paymentAmount)} paid` : 'No Payment Recorded'}</small></div><div><span className={`appointment-state ${slug(appointmentStatus(job))}`}>{appointmentStatus(job)}</span>{proximity && <small className="appointment-nearest-truck">{proximity}</small>}{['Completed', 'Estimate Closed'].includes(appointmentStatus(job)) && <small>On-site: {job.onsiteTime?.label || 'Unavailable'}</small>}<small>{appointmentStatus(job) === 'Canceled' ? job.cancellationReason : job.appointmentNotes.join(' · ')}</small></div></article>; })}</section>; })}
        </section>;
