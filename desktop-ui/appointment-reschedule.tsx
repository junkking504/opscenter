...
export function AppointmentReschedule({job,date,saved,onBusyChange,onOpenDate}:{job:ScheduleAppointment;date:string;saved:(destination:string)=>void;onBusyChange:(busy:boolean)=>void;onOpenDate?:(date:string)=>void}) {
  const restore=/cancel(?:ed|led)/i.test(job.status);
  const actionLabel=restore?'Restore Appointment':'Reschedule Appointment';
  ...
  const valid=validDate && start!=='' && Number(start)%60===0 && duration>0 && duration<=720 && duration%60===0 && Number(start)+duration<=1440 && (restore || destination!==date || Number(start)!==job.appointmentStartMinutes);
  ...
  const confirm=async()=>{
    ...
    try {
      const result=await sendScheduleChange({...job,version:reviewVersion},date,restore?'restore':'reschedule',{destinationDate:destination,appointmentStartMinutes:Number(start)},requestId);
      setReceipt(result);
  ...
  return <section id="appointment-reschedule" className="drawer-reschedule" aria-label={actionLabel}>
    ...
    {restore && <p>Restore this canceled appointment to Confirmed on the selected date. Review truck assignment after restoring.</p>}
    ...
    {!review && !receipt && <Button variant="outline" disabled={blocked || !valid} onClick={()=>{setReviewVersion(job.version);setReview(true);}}>{restore?'Review Restoration':'Review Reschedule'}</Button>}
    {review && !receipt && <div className="drawer-reschedule-review" role="group" aria-label={restore?'Review restoration':'Review reschedule'}>
      ...
      <p>{restore?'Confirming restores this canceled appointment in JunkWare.':'Confirming changes this appointment in JunkWare.'}</p>
      ...
      <div className="drawer-quick-actions"><Button variant="outline" disabled={busy} onClick={()=>setReview(false)}>{restore?'Keep Canceled':'Keep Appointment'}</Button><Button disabled={blocked || !valid || reviewChanged} onClick={()=>void confirm()}>{busy?'Verifying in JunkWare…':restore?'Confirm Restoration':'Confirm Reschedule'}</Button></div>
    </div>}
