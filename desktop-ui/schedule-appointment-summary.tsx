...
export default function ScheduleAppointmentSummary({job,closest,loading,isToday,busy,open,clear}: {
  ...
  return <section className="schedule-appointment-summary" aria-label={`Selected job ${job.jkNumber}`}>
    ...
    <aside className="selected-job-next">
      <div className="selected-job-closest" aria-live="polite"><span>Closest truck now</span>{!isToday ? <strong>Available for today only</strong> : !job.location ? <strong>Verify address first</strong> : loading ? <strong>Checking distance…</strong> : closest ? <><strong>{truckLabel(closest.truck)} <span>{closest.minutes} min · {closest.miles} mi</span></strong><small>Recent GPS · road estimate. Check availability before assigning.</small></> : <><strong>Unavailable</strong><small>No current GPS and road estimate available.</small></>}</div>
