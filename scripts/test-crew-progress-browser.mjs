...
try {
  ...
  const labels = () => timeline.locator('.crew-update-content header strong').allTextContents();
  const expected = ['Arrival','New Appointment','Clock In','Duration','Job Closed'];
  ...
  const closed = timeline.locator('.crew-update').filter({hasText:'Job Closed'});
  ...
  assert.deepEqual(await labels(),['Duration','Job Closed']);
  await page.getByRole('button',{name:'Clear filters',exact:true}).click();
