...
export function readDesktopFinance(date: string): FinanceData & { wexFuel: ReturnType<typeof readWexFuelFinance>; dumpExpenses: ReturnType<typeof readDumpExpenses> } {
  ...
  return { dumpExpenses: readDumpExpenses(date), recyclingReceipts: readRecyclingData().receiptDrafts || [], statements: readFinancialStatements(), wexFuel, comparison:financePeriodComparison(monthly.range.dataThroughDate,readDaily), date, available: Boolean(metrics), generatedAt: metrics?.generated_at || metrics?.updated_at || null,
    daily: dailySummary,
